...
# ─── CREDENTIALS (live) ───
# The checks above only prove a variable is set. These prove it still works:
# an expired LLM key or a revoked app password passes every check above and
# installs silently, then shows up as mail captured but never classified.
# Set DOCTOR_SKIP_LIVE=1 to skip (offline runs, CI without egress).
echo "CREDENTIALS (live)"

if [ "${DOCTOR_SKIP_LIVE:-0}" = "1" ]; then
  skip "Live credential checks skipped (DOCTOR_SKIP_LIVE=1)"
elif ! command -v python3 > /dev/null 2>&1; then
  warn "python3 not found — cannot verify IMAP/SMTP/LLM credentials"
elif [ ! -f scripts/check-credentials.py ]; then
  warn "scripts/check-credentials.py missing — credentials not verified"
else
  while IFS='|' read -r status message; do
    case "$status" in
      ...
      FAIL) fail "$message" ;;
      SKIP) skip "$message" ;;
      *)    [ -n "$status" ] && warn "$status$message" ;;
    esac
  done < <(python3 scripts/check-credentials.py 2>&1)
fi
