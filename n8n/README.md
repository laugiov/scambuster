...
account (e.g., `WF-INTAKE-EMAIL-beta-counsel`), verify that the
...
4. Activate the 3 trigger workflows: `WF-INTAKE-EMAIL-V2`, `WF-REPLY-SEND-v1`, `WF-EXTRACT-AND-ENRICH-IOC`. `WF-REPLY-GENERATE-V2` is a sub-workflow (no trigger node) and is not independently activated — it runs when invoked by `WF-INTAKE-EMAIL-V2`.
...
For email (IMAP/SMTP), see `docs/17_email_provider_setup.md`.
