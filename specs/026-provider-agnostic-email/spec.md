...
- **WF-INTAKE-EMAIL-V2** uses `n8n-nodes-base.gmailTrigger` (node: "Gmail Trigger") to poll for new emails, then `n8n-nodes-base.gmail` (node: "Gmail Get Raw Message") to fetch the full RFC822 content. Both require a `gmailOAuth2` credential (`id: credential-placeholder-1`, "Gmail account Operator") configured via Google Cloud Console OAuth -- a 45-minute setup involving project creation, OAuth consent screen, credential generation, and token exchange.
...
Gmail Trigger (gmailTrigger v1.1)
  ...
  - credentials: gmailOAuth2 "Gmail account Operator"
