...
## Recent Changes
- 064-link-attachment-sha256: Closed spec 063 SC-008 follow-up. New private IngestHandler::linkAttachmentsAsIocs() called after em->flush() iterates Message::getAttachments() and calls IocUpsertService::upsertEnrichedIoc() with type='sha256', source='attachment', tlp='AMBER', enrichment={attachment_id, filename, mime_type, size_bytes}. Optional ?IocUpsertService injected via Symfony autowiring (backwards-compat). Defensive try/catch on InvalidArgumentException (spec 061 outgoing-message guard, silent DEBUG) and \Throwable (WARNING). +3 EndToEnd tests on IngestControllerTest including a sentinel that documents the existing UNIQUE(content_hash) attachment dedup limitation (same PDF in 2 mails → 1 attachment row + 1 observed_ioc, not 2 — known architectural quirk pre-dating spec 063, future fix would require many-to-many message_attachment join table). 13-assertion test_ingest_raw_with_attachments sentinel and NoIocFromOutgoingMessageTest both pass unchanged. PHPStan L6 src clean. Full backend suite 2430+41 E2E green. Smoke validated on dev: 1 PDF via curl → sha256 row in observed_ioc within seconds.
- 063-backend-side-attachment: Restored end-to-end attachment capture lost since 2026-03-31 (commit b090e31, Gmail→IMAP migration). 4 atomic commits: (1) backend EmailParsingService::extractAttachments() with zbateson/mail-mime-parser + 25MB chunked-stream guard; (2) IngestHandler::ingest() hybrid fallback that invokes the parser only when dto.attachments empty (DTO precedence preserved, test_ingest_raw_with_attachments sentinel unchanged); (3) PayloadSizeLimitListener path-aware: 1MB default + 50MB on /api/v1/communication/ingest/* with structured 413 logging + receivedBytes/hint in response, plus docker-compose NODE_FUNCTION_ALLOW_BUILTIN=crypto for n8n vm2 sandbox; (4) WF-INTAKE-EMAIL-V2 IMAP node format=simple+downloadAttachments=true (NOT raw — n8n raw mode is broken, returns body without headers per RFC3501 BODY[TEXT]) + Extract Email Data V5 reads $input.item.binary and computes sha256 via require('crypto'). +12 unit tests + 4 E2E + 2 listener tests = +18 tests. Validated end-to-end on dev with 1 PDF + 3 STIX bundles (one 1.01MB) via real n8n IMAP pipeline. Known follow-up: file hashes not yet linked into observed_ioc (separate gap, ~spec 064 future).
