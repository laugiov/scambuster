...
## Recent Changes
- 063-backend-side-attachment: Restored attachment capture lost since 2026-03-31 (commit b090e31, Gmail→IMAP migration). New EmailParsingService::extractAttachments() uses zbateson/mail-mime-parser to extract attachments from raw multipart RFC822 (uses getAllAttachmentParts() + inline exclusion + chunked stream read with 25MB default size limit). IngestHandler::ingest() invokes the parser fallback only when dto.attachments is null/empty — DTO precedence preserved (existing test_ingest_raw_with_attachments sentinel passes unchanged). 12 new unit tests + 4 new EndToEnd tests. Manual smoke validated with 1 PJ + 3 PJ via curl. Known follow-up: file hashes not yet linked into observed_ioc (separate gap, ~spec 064). n8n side not updated by this spec — backend is now self-sufficient if upstream forwards full RFC822 raw bytes.
