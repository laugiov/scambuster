...
make quickstart
```
This builds the images, starts the stack, runs migrations + fixtures, generates JWT
keys, loads a demo dataset, and **auto-configures n8n** (admin account, workflow
import + activation, and the IMAP credential — all derived from `.env`). No manual
n8n UI step is needed for the default mailbox.
