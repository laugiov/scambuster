...
- **Recall.** Nothing here says how many behaviours the extractor missed. Measuring
  that needs exhaustive annotation of full messages, which is the job of the
  annotated public dataset under `dataset/`.
...
- **Anything beyond the sample.** The figures are descriptive counts. There is no
  extrapolation to the full corpus and no confidence interval.
...
**The published overall figure comes from a uniform draw.** The honest reading of an overall precision number
...
discussion until both sheets are complete. Each appends the six scoring
...
`adjudication_reason`. Adjudication happens after both sheets are complete,
never before.
...
- **The annotated public dataset** shares the codebook, so the dataset labels and
  these verdicts are produced under one set of rules.
- **Standards-track submissions** have to cite a quality figure and its limits. Until
  section 3 is filled, nothing is filed.
- Public texts and conference material can quote the figure — with the method and the
  limits attached, never the number alone.
