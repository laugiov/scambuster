...
| Persona diversity | 27 personas across 7 archetypes with distinct writing styles | DB-seeded personas (Doctrine fixtures) |
...
| Kill switch | Immediate halt available at workflow, API, database, and infrastructure levels | `ReplyCadenceService` (reads `SCAMBUSTER_KILL_SWITCH`), `ToggleLlmKillSwitchController` (API toggle) |
...
| JWT key on filesystem | JWT tokens signed with RS256 using the RSA private key at `config/jwt/private.pem`, decrypted with `JWT_PASSPHRASE` (never a shared secret in env) | `lexik/jwt-authentication-bundle` config (`lexik_jwt_authentication.yaml`) |
...
|---------|-------------------|----------------|
| Kill switch (4 levels) | T1, T2, T3, T4, T5 | `ReplyCadenceService` (`SCAMBUSTER_KILL_SWITCH`) + `ToggleLlmKillSwitchController`, n8n, DB flags, container stop |
