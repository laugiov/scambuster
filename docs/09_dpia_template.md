...
- Tags scammer tactics, techniques and procedures (TTPs) observed in inbound messages against a MITRE ATT&CK-aligned taxonomy, storing a verbatim evidence excerpt (with character offsets) and a confidence score per observation
...
| Scammer TTP observations | ATT&CK-aligned technique code (via lkp_ttp), extraction confidence, verbatim evidence excerpt + character offsets, taxonomy/model/prompt provenance | Extracted from inbound scam emails | Verbatim evidence excerpt aligned with email content retention; technique code + metadata indefinite (intelligence value, as IOCs) |
...
- **Automation is necessary** because manual scambaiting does not scale (one operator can handle only a handful of concurrent conversations; the system handles many in parallel)
...
| R6 | Cost explosion (LLM API) | Medium | Low | Rate limiting across the LLM pipeline (replies/conversation, LLM calls/hour, active conversations/day) plus per-sender and per-account ingest caps |
...
| Rate limiting | Redis-backed Symfony rate-limiter with 8 named limiters: login (IP + email), API (global token bucket), LLM pipeline (replies/conversation, calls/hour, active conversations/day), per-sender ingest, per-account ingest |
...
| Measure | Description |
|---------|-------------|
| Access control | JWT RS256 authentication (15min TTL), RBAC with 13 fine-grained permissions, PermissionVoter |
| Audit trail | Append-only audit_log table with 33 event types, trace_id correlation, IP capture |
...
| Code review | DDD architecture with strict layer separation, PHPStan level 8 |
...
- [x] Technical and organizational measures are in place
