...
These default to safe values; set them only to change behaviour:

| Variable | Default | Effect when disabled |
|----------|---------|----------------------|
| `SCHEDULER_ENABLED` | `true` | The `scheduler` container runs no background tasks. |
| `TTP_EXTRACTION_ENABLED` | `true` | Turns off the scammer-TTP module: the ingest hook becomes a no-op (ingestion, IOC extraction and replies are unaffected) and the manual `POST /communication/message/{msgId}/extract-ttps` endpoint returns **503**. Already-stored observations and the read/export endpoints stay available; re-enable and run `scambuster:ttp:backfill` to catch up on messages missed while it was off. |
...
The dashboard will show 36 synthetic conversations with realistic IOCs. All LLM calls return mock responses.
...
ScamBuster resolves the destination account from each inbound message's **recipient
address**, so mailboxes are added by configuration alone — no workflow or code edit.

### Single-mailbox install (default)

`make quickstart` registers your honeypot mailbox automatically from the
`HONEYPOT_IMAP_USER` value in your `.env`, so inbound mail resolves to it out of
the box. Outbound replies use the global `MAILER_DSN`. If you set
`HONEYPOT_IMAP_USER` *after* seeding, register it once with the command below.

### Adding additional mailboxes

For each additional mailbox, register it via the CLI (the `--email` must exactly
match the address that mailbox receives mail at — matching is case-insensitive):
...
your mailboxes. The command stores the reply-from address and per-account SMTP DSN.
In the n8n UI, add an IMAP credential for the new mailbox and point the intake
workflow's IMAP node at it (duplicate the intake workflow only if you poll several
mailboxes on independent schedules). **No node code changes** — the intake workflow
forwards each message with an empty `account_id` and the backend derives the account
from the recipient. See [17_email_provider_setup.md](17_email_provider_setup.md) for
...
│   │   ├── DataFixtures/      # Seed data (14 scam types, 27 personas)
...
| Backfill rewards | Every 6h | `app:rewards:calculate` |
...
- Verify the `JWT_PASSPHRASE` in `.env` matches what was used when the keys were generated.
