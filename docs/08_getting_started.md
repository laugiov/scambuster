...
address**, so no node logic or backend code changes when you add a mailbox. What
each new mailbox *does* need is two things: a row in the backend, and its own IMAP
trigger in n8n. A single `IMAP Email Trigger` node binds exactly **one** IMAP
credential, so one node can only ever poll one mailbox.
...
> **Do step 1 before step 2.** If mail arrives on a mailbox that has no backend
> row, ingestion throws `Unknown account_id` and the message is **lost**: the IMAP
...
docker compose exec backend-dev bin/console app:mail-account:add \
    ...
    --label="Internal name shown in the dashboard"
```
...
See [17_email_provider_setup.md](17_email_provider_setup.md) for the full
walkthrough.
