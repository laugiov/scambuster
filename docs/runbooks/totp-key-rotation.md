...
> **Spec 065e** — operational guide for managing `TOTP_ENCRYPTION_KEY`.

## Background

Since spec 065e (April 2026), `app_users.totp_secret` is encrypted at
rest using libsodium `secretbox` with the `TOTP_ENCRYPTION_KEY` env var
as the key. The key is a 32-byte random value stored as 64 hex chars
in `.env`.
...
- Spec 065e: `specs/065e-sprint2-identity-hardening/`
