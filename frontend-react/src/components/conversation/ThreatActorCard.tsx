  ...
  return (
    <div className="bg-surface-low rounded-lg border border-white/5 overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <span
            className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium uppercase tracking-wider ${sophStyle}`}
          >
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3">
          ...
          {/* Description */}
          {profile.description && (
            <p className="text-xs text-on-surface-variant leading-relaxed">{profile.description}</p>
          )}

          {/* Metrics */}
          <div className="border-t border-white/5 pt-2">
            <MetaRow label="Engagement">
              {formatHours(profile.engagementHours)} / {profile.engagementTurns} turns
            </MetaRow>
