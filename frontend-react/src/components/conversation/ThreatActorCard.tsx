...
export function ThreatActorCard({ profile, personaLabel }: { profile: ThreatActorProfile; personaLabel?: string }) {
  ...
  const isCluster = profile.clusterType === 'consolidated';
    ...
    <div className="bg-surface-low rounded-lg border border-white/5 overflow-hidden">
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3">
          ...
          {/* Metrics — a consolidated cluster describes a GROUP, so per-session
              engagement/IOC-diversity are meaningless (always 0). Show cluster-scoped
              facts instead; single-conversation actors keep the engagement metrics. */}
          <div className="border-t border-white/5 pt-2">
            {isCluster ? (
              <>
                <MetaRow label="Conversations">{profile.conversationCount}</MetaRow>
                {profile.anchorIocTypes.length > 0 && (
                  <MetaRow label="Shared IOCs">
                    <span className="inline-flex flex-wrap gap-1 justify-end">
                      {profile.anchorIocTypes.map((iocType) => (
                        <span key={iocType} className="px-1.5 py-0.5 bg-info/15 text-info rounded text-xs">
                          {iocType}
                        </span>
                      ))}
                    </span>
