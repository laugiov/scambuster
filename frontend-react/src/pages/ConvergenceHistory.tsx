...
export function ConvergenceHistory() {
  ...
  if (error)
    return <ErrorMessage message={t('convergence.failedLoad')} onRetry={() => void refetch()} />;

  const entries = data?.by_scam_type ?? {};

  // Flatten and sort by date descending, then scam type
  const rows = Object.entries(entries)
    .flatMap(([scamType, logs]) => logs.map((log) => ({ ...log, scam_type: scamType })))
      ...
      <header className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 text-xs text-on-surface-dim">
          ...
          <span className="text-success">
            {t('convergence.converged', { count: convergedCount })}
          </span>
            ...
            {paged.map((row, i) => (
              ...
              >
                ...
                <td className="px-5 py-3 font-mono text-on-surface">
                  {(row.dominant_pct * 100).toFixed(1)}%
                </td>
