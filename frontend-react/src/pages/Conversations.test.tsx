...
const mockMailAccounts = [
  { account_id: 'acc-tarrowby', label: 'Tarrowby Holdings', email: 'admin@tarrowbyholdings.com' },
  { account_id: 'acc-calverton', label: 'Calverton Partners', email: 'admin@calvertonpartners.com' },
  { account_id: 'acc-aldridge', label: 'Aldridge Counsel', email: 'admin@aldridgecounsel.com' },
];
...
const mockConversations = [
  {
    ...
    account_label: 'Tarrowby Holdings',
    account_email: 'admin@tarrowbyholdings.com',
  },
  {
    conv_id: 'conv-2',
    status: 'open',
  ...
  },
  {
    ...
    account_label: 'Calverton Partners',
    account_email: 'admin@calvertonpartners.com',
  },
];
...
describe('Conversations page', () => {
  ...
  // ──────────────────────────────────────────────────────────────────────
  // Spec 087 — US2: mailbox column on the conversation list
  // ──────────────────────────────────────────────────────────────────────
  ...
  it('renders a MAILBOX column header between PERSONA and RISK', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Tarrowby Holdings/).length ?? 0).not.toBe(0);
  ...
  it('renders the mailbox label in each conversation row', async () => {
    ...
    await waitFor(() => {
      // 2 fixture conversations are linked to Tarrowby → 2 cells in tbody
      expect(within(tbody).getAllByText('Tarrowby Holdings').length).toBe(2);
    });
    expect(within(tbody).getByText('Calverton Partners')).toBeInTheDocument();
  });

  // ──────────────────────────────────────────────────────────────────────
  // Spec 087 — US3: mailbox dropdown filter
  // ──────────────────────────────────────────────────────────────────────
  ...
  it('renders the mailbox dropdown with all active mailboxes', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText('Tarrowby Holdings').length).toBeGreaterThan(0);
    });
    ...
    expect(optionLabels).toContain('Tarrowby Holdings');
    expect(optionLabels).toContain('Calverton Partners');
    expect(optionLabels).toContain('Aldridge Counsel');
  ...
  it('filters conversations to a single mailbox when selected', async () => {
    ...
    await user.selectOptions(dropdown, 'Tarrowby Holdings');

    await waitFor(() => {
      // Calverton row should disappear from the table body (still present in dropdown)
      expect(within(tbody).queryByText('Calverton Partners')).not.toBeInTheDocument();
    });
    expect(within(tbody).getAllByText('Tarrowby Holdings').length).toBe(2);
  });
});
