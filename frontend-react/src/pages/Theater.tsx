...
import { TheaterKillChainStrip } from '@/components/theater/TheaterKillChainStrip';
...
function TheaterContent({ data }: { data: NonNullable<ReturnType<typeof useTheaterReplay>['data']> }) {
  ...
  // `?stage=1` enters presenter mode: screen-share on,
  ...
  // apply `?stage=1` once on mount: mask on (default
  ...
  // chapter markers on the progress bar. We derive one
  ...
  // typing direction is DERIVED from state (not dispatched).
