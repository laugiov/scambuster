...
export function Analytics() {
  ...
  if (error)
    return (
      <ErrorMessage
        message={t('analytics.failedLoad')}
        onRetry={() => void iocTimeline.refetch()}
      />
    );
            ...
            <div className="grid grid-cols-2 gap-3 h-full overflow-auto">
              {convergenceSparklines.map((spark) => (
                <div key={spark.scamType} className="bg-surface-base rounded p-2">
                  <span className="text-[10px] text-on-surface-dim block mb-1">
                    {spark.scamType}
                  </span>
...
function buildConvergenceSparklines(
  byScamType: Record<string, { date: string; dominant_pct: number }[]>,
): SparklineData[] {
