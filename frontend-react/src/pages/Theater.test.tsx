...
describe('Theater page — keyboard navigation', () => {
  ...
  it('collapses LLM sub-block when all signals are zero', async () => {
  ...
  it('renders full LLM sub-block when at least 2 signals are non-zero', async () => {
    const fix = fixture();
    // tighter heuristic: need ≥2/3 signals non-zero
  ...
  it('renders teaser at step 0 idle, hidden once playback advances', async () => {
    renderTheater();
  ...
  it('hides first-financial spoiler until visibleStep reaches turn', async () => {
  ...
  it('cascade_events counter ticks up live (not pre-spoiled at step 0)', async () => {
    const fix = fixture();
  ...
  it('?stage=1 keeps mask on + shows stage chip in the banner', async () => {
    server.use(
      http.get(`${BASE}/communication/conversation/${CONV_ID}/theater`, () => HttpResponse.json(fixture())),
    );
  ...
  it('renders chapter markers on the progress bar', async () => {
