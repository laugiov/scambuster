...
describe('Personas', () => {
  ...
  // KPI gate. The previous reduce() walked all personas and
  ...
  // cold-start one.
  it('KPI excludes cold-start personas from the headline', async () => {
    server.use(
      http.get(`${BASE}/meta/config`, () => HttpResponse.json(mockMetaConfig)),
      ...
      // KPI showed "—" and assertions on it were never written). The KPI
      // gate needs real data to test, so we wrap properly here.
      http.get(`${BASE}/scambaiting/persona/:code/performance`, ({ params }) => {
  ...
  it('KPI shows the no-qualified caption when every persona is cold-start', async () => {
    server.use(
      http.get(`${BASE}/meta/config`, () => HttpResponse.json(mockMetaConfig)),
      http.get(`${BASE}/scambaiting/persona/:code/performance`, ({ params }) =>
        HttpResponse.json({
