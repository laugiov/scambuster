...
import { sophisticationBadge, riskText } from '@/lib/actorColors';
...
import { PsychProfilePanel } from '@/components/clusters/PsychProfilePanel';
import { ClusterTtpPanel } from '@/components/clusters/ClusterTtpPanel';
import { TemporalPanel } from '@/components/clusters/TemporalPanel';
import { AbuseReportPanel } from '@/components/clusters/AbuseReportPanel';
...
// The four deep-linkable Cluster Detail tabs (?tab=…). Anything else in the URL
// resolves to Overview — the URL itself is never rewritten.
const CLUSTER_TABS = ['overview', 'ttps', 'indicators', 'campaigns'] as const;
type ClusterTab = (typeof CLUSTER_TABS)[number];

function isClusterTab(value: string | null): value is ClusterTab {
  return (CLUSTER_TABS as readonly string[]).includes(value ?? '');
}
...
export function ClusterDetail() {
  ...
  // no URL-normalizing effect: an invalid or absent ?tab= renders Overview while
  // the URL stays untouched.
  const tabParam = searchParams.get('tab');
  const activeTab: ClusterTab = isClusterTab(tabParam) ? tabParam : 'overview';

  // Switching the tab sets ?tab= while preserving any other query params.
  const selectTab = (tab: ClusterTab) => {
  ...
  const tabs: { id: ClusterTab; label: string }[] = [
    { id: 'overview', label: t('clusterDetail.tabs.overview') },
    { id: 'ttps', label: t('clusterDetail.tabs.ttps') },
    { id: 'indicators', label: t('clusterDetail.tabs.indicators') },
    { id: 'campaigns', label: t('clusterDetail.tabs.campaigns') },
  ];
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-1 text-xs text-on-surface-dim">
            {cluster.sophistication && cluster.sophistication !== 'minimal' && (
              <>
                <span
                  title="How sophisticated this actor's operation is (minimal → intermediate → advanced), inferred from their tradecraft, infrastructure and volume."
                  className={`rounded-full border px-2 py-0.5 text-[11px] font-medium capitalize ${sophisticationBadge(cluster.sophistication)}`}
                >
      ...
      {/* Tab nav — page-level primary tabs, deep-linkable via ?tab= */}
      ...
      {/* Overview: verdict + cluster panels + behavioural summary */}
      {activeTab === 'overview' && (
        <div className="space-y-6">
          <VerdictBanner cluster={cluster} />

          <PsychProfilePanel clusterId={id ?? ''} />

          <TemporalPanel clusterId={id ?? ''} />
      ...
      {/* TTPs */}
      {activeTab === 'ttps' && <ClusterTtpPanel clusterId={id ?? ''} />}

      {/* Indicators & Conversations — kept on one tab: selecting an anchor IOC
          filters the conversation list below (selectedIndicatorId drives
          filteredConversations), so the two lists must live together. */}
                ...
                {(showAllConversations
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 text-xs text-on-surface-dim">
                      ...
                      <span title="Risk score (0–100) for this conversation. Coloured red ≥ 80, amber ≥ 50.">Risk: <span className={`font-semibold ${riskText(conv.score_risk)}`}>{conv.score_risk}</span></span>
                    </div>
                  </Link>
                ))}
