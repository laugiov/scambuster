...
describe('ConversationDetail', () => {
  ...
  // ──────────────────────────────────────────────────────────────────────
  // mailbox visible in SESSION METADATA
  // ──────────────────────────────────────────────────────────────────────
  ...
  // ──────────────────────────────────────────────────────────────────────
  // manual close / reopen (spec 144)
  // ──────────────────────────────────────────────────────────────────────

  it('shows the Close button for an open conversation and posts on confirm', async () => {
    let closed = false;
    setupHandlers();
    server.use(
      http.post(`${BASE}/scambaiting/conversation/${CONV_ID}/close`, () => {
        closed = true;
        return HttpResponse.json({ success: true, conv_id: CONV_ID });
      }),
    );
    const user = userEvent.setup();
    render(<ConversationDetail />, { wrapper: createWrapper() });

    const btn = await screen.findByRole('button', { name: /close conversation/i });
    await user.click(btn);
    // Warning must be shown before the action fires.
    expect(screen.getByText(/finalizes the baiting session/i)).toBeInTheDocument();

    await user.click(screen.getByRole('button', { name: 'Confirm' }));
    await waitFor(() => expect(closed).toBe(true));
  });

  it('shows the Reopen button and its bandit warning for a closed conversation', async () => {
    server.use(
      http.get(`${BASE}/communication/conversation/${CONV_ID}`, () =>
        HttpResponse.json({ ...mockConvDetail, status: 'closed' })),
      http.get(`${BASE}/communication/conversation/${CONV_ID}/messages`, () => HttpResponse.json(mockMessages)),
      http.get(`${BASE}/communication/conversation/${CONV_ID}/iocs`, () => HttpResponse.json(mockIocs)),
      http.get(`${BASE}/communication/conversation`, () => HttpResponse.json(mockConversations)),
      http.get(`${BASE}/meta/config`, () => HttpResponse.json(mockMetaConfig)),
    ...
    const btn = await screen.findByRole('button', { name: /reopen conversation/i });
    await user.click(btn);
    expect(screen.getByText(/count the outcome a second time/i)).toBeInTheDocument();
  });
});
