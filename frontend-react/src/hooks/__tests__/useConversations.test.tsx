...
describe('useCloseConversation', () => {
  it('POSTs to the close endpoint', async () => {
    let hit = false;
    server.use(
      http.post(`${BASE}/scambaiting/conversation/conv-x/close`, () => {
        hit = true;
        return HttpResponse.json({ success: true, conv_id: 'conv-x' });
      }),
    );

    const { result } = renderHook(() => useCloseConversation(), { wrapper: createWrapper() });
    await act(async () => {
      result.current.mutate('conv-x');
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
...
describe('useReopenConversation', () => {
  it('POSTs to the reopen endpoint', async () => {
    let hit = false;
    server.use(
      http.post(`${BASE}/scambaiting/conversation/conv-y/reopen`, () => {
        hit = true;
        return HttpResponse.json({ success: true, conv_id: 'conv-y' });
      }),
    );

    const { result } = renderHook(() => useReopenConversation(), { wrapper: createWrapper() });
    await act(async () => {
      result.current.mutate('conv-y');
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(hit).toBe(true);
  });
