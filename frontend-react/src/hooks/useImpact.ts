...
interface WastedTime {
  ...
  // Scammer Replies Elicited tile. Count of inbound messages
...
interface IocValue {
  ...
  novel_iocs: number;
  novel_pct: number;
  // Fresh IOCs tile, replaces the misleading Novel % tile.
...
export function useImpactSummary(period: string = 'all', scamType?: string | null) {
  return useQuery<ImpactSummary>({
    ...
    queryFn: async () => {
      // scam_type combines with period (orthogonal filters).
...
export function useIocUniqueness(period: string = '30d', scamType?: string | null) {
  return useQuery<IocUniquenessData>({
    ...
    queryFn: async () => {
      // scam_type combines with period.
      const params: Record<string, string> = { period };
      if (scamType) params.scam_type = scamType;
...
// Bias-corrected scammer engagement metric
...
export function useScammerEngagement(
  ...
  if (scamType) params.scam_type = scamType;
  // period filter combines with scam_type orthogonally.
