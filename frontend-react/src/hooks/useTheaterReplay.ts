...
import type { StimulusType } from '@/lib/stimulusLabels';

/**
 * Live Bait Theater data hook.
 *
...
export interface TheaterMeta {
  ...
  /** human-readable persona label (preferred for display). */
  ...
  iocs_count: number;
  /** count of actionable IOCs (excludes header artifacts).
...
/**
 * A confirmed scammer TTP observed on a message. Evidence is OFFSETS-ONLY
 * (UTF-8 code-point [start, end) into `subject + "\n\n" + body_text`); the
 * verbatim quote is never sent — the client reconstructs the highlight.
 */
export interface TheaterTtp {
  msg_id: string;
  ttp_code: string;
  ttp_label: string;
  phase: string;
