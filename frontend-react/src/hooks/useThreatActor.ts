...
// STIX 2.1: the x_scambuster_actor payload now lives under its extension-definition id
// as a property-extension. Keep a fallback to the legacy bare key for robustness.
const ACTOR_EXT_ID = 'extension-definition--c3b48d34-52e8-4f3a-ad9b-2b6a7e4f9c01';
...
function extractProfile(bundle: StixBundle): ThreatActorProfile | null {
  ...
  const wrapped = actor.extensions?.[ACTOR_EXT_ID]?.x_scambuster_actor as Record<string, unknown> | undefined;
  const ext = wrapped ?? actor.extensions?.x_scambuster_actor;
  ...
  return {
    ...
    // Consolidated-cluster fields (present only on the clustered-actor export).
    clusterType: (ext?.cluster_type as string) ?? null,
    conversationCount: (ext?.conversation_count as number) ?? 0,
    anchorIocTypes: Array.isArray(ext?.anchor_ioc_types) ? (ext.anchor_ioc_types as string[]) : [],
  };
}
