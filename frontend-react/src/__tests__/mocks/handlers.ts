...
export const handlers = [
  ...
  // TTP surfaces default to "no observations" so cluster/conversation detail
  // page tests render cleanly; TTP-specific tests override with server.use().
  http.get(`${BASE}/clusters/:id/ttps`, ({ params }) =>
    HttpResponse.json({ cluster_id: String(params.id), ttps: [], top_sequences: [] })),
  http.get(`${BASE}/conversations/:id/ttps`, ({ params }) =>
    HttpResponse.json({ conv_id: String(params.id), observations: [], timeline: [] })),
  ...
  http.get(`${BASE}/iocs/:id/ttps`, ({ params }) =>
    HttpResponse.json({ ioc: String(params.id), ttps: [] })),
