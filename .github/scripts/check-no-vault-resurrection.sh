#!/usr/bin/env bash
# Spec 065c — Prevent re-introduction of Vault dead code.
# This script fails the CI if any Vault reference appears in src/.
#
# Rationale: Vault was removed in spec 065c (April 2026) because it was
...
if grep -rln "$PATTERN" backend-symfony/src/ 2>/dev/null; then
    ...
    echo ""
    echo "Vault was removed in spec 065c (Sprint 1 of the security & quality"
    echo "roadmap). The n8n IMAP intake holds the production credentials now."
    echo ""
