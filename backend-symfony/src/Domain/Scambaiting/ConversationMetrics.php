...
final readonly class ConversationMetrics implements \Stringable
{
     ...
     * @param int  $iocsSensibles Number of high-value IOCs (IBAN, phone, crypto) (>= 0)
    ...
    public function calculateReward(): float
    {
        ...
        // Ensure reward ∈ [0.0, 1.0] (guard against calculation errors)
        return max(0.0, min(1.0, $reward));
    }

    /**
     * Normalizes a value into [0, 1] using min-max scaling.
     *
    ...
    private function validate(): void
    {
        ...
        if ($this->iocsSensibles < 0) {
            throw new \InvalidArgumentException(
                "Sensitive IOCs must be >= 0, got {$this->iocsSensibles}"
            );
        }

        if ($this->iocsSensibles > $this->iocsTotal) {
            throw new \InvalidArgumentException(
                "Sensitive IOCs ({$this->iocsSensibles}) cannot exceed total IOCs ({$this->iocsTotal})"
            );
        }
    }
