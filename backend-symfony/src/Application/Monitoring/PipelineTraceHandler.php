...
final class PipelineTraceHandler
{
    ...
    public function getRecentTraces(
        ...
        $where = "m.direction = (SELECT dir_id FROM lkp_direction WHERE code = 'out') AND m.ts_msg > NOW() - INTERVAL '{$days} days' AND jsonb_exists(m.headers::jsonb, 'pipeline_trace')";
        $params = [];
    ...
    public function getTraceByMessageId(string $msgId): ?array
    {
        $sql = <<<'SQL'
            ...
            FROM message m
            WHERE m.msg_id = :msgId AND m.direction = (SELECT dir_id FROM lkp_direction WHERE code = 'out')
            SQL;
    ...
    public function getHealthMetrics(int $hours = 24): array
    {
        ...
        $sql = <<<SQL
            SELECT m.headers->'pipeline_trace' as pipeline_trace
            FROM message m
            WHERE m.direction = (SELECT dir_id FROM lkp_direction WHERE code = 'out')
