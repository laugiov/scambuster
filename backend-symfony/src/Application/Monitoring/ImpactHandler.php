...
final readonly class ImpactHandler
{
     ...
     *
     * Accepts optional `$scamType` to filter all metrics by
     * scam type code (e.g. INVOICE_FRAUD). When null, behavior is byte-identical
     * to the unfiltered response (regression guarantee for `All` filter).
     *
    ...
    public function getSummary(string $period = 'all', ?string $scamType = null): array
    {
        ...
        return [
            // getWastedTime also derives a period-aware
            ...
            // fresh_iocs window inside ioc_value follows the
    ...
    /**
     * Normalize the scam_type query param. Trims whitespace and
    ...
    /**
     * Sub-query fragment selecting `scam_type_id` from
    ...
    public function getIocUniqueness(string $period = '30d', ?string $iocType = null, ?string $scamType = null): array
    {
        ...
        // Sub-query filter for scam_type via observed_ioc → message → conversation.
        ...
        // Daily trend respects the period threshold when set;
    ...
    private function getWastedTime(?string $threshold, ?string $scamType = null, string $period = 'all'): array
    {
        ...
        // Resolve the inbound direction id once. dir_id varies
        ...
        // Qualified conversations only: turns_count >= 2 means
        ...
        // Weekly trend respects the period threshold when set;
        ...
        // Same qualified-conversation filter as the headline.
        $trendRows = $this->connection->fetchAllAssociative(
        ...
        // "Scammer Replies Elicited" tile: direct count of
        // inbound messages (direction='in') in qualified conversations.
        // Reframed from the previous time-based metric after a methodology
        // audit that led to the switch from time → count.
    ...
    private function getIocValue(?string $threshold, ?string $scamType = null, string $period = 'all'): array
    {
        ...
        // When scam_type is provided, narrow the indicator set
        ...
        // "Fresh IOCs" honest replacement for the misleading
    ...
    private function getCostEfficiency(?string $threshold, ?string $scamType = null): array
    {
        ...
        // When scam_type is provided, llm_usage rows are filtered by
        ...
        // Sub-query filter for indicator table when scam_type set
        $scamIdSubquery = null !== $scamType
            ? ' AND indicator_id IN (SELECT DISTINCT oi.indicator_id FROM observed_ioc oi'
            . ' JOIN message m ON oi.msg_id = m.msg_id'
            . ' JOIN conversation c ON m.conv_id = c.conv_id'
    ...
    {
        // When scam_type set, narrow campaigns to those whose
    ...
    private function computeTrends(string $period, ?string $scamType = null): ?array
    {
        ...
        // Scam_type filter fragments reused across all trend queries.
        $params = null !== $scamType ? ['scam_type' => $scamType] : [];
        $convScamFilter = null !== $scamType ? ' AND scam_type_id = ' . $this->scamTypeIdLookupSubquery() : '';
        ...
        // Current wasted hours (qualified convs only, must match
