...
/**
 * Campaign Profiler - LLM-based campaign analysis
 *
...
final readonly class CampaignProfiler
{
     ...
     *
     * @param array<Message> $sampleMessages Sample of 3-10 messages
     *
    ...
    public function profile(array $sampleMessages): array
    {
        if (count($sampleMessages) < 3) {
            throw new \InvalidArgumentException('At least 3 messages are required to profile a campaign');
        }
    ...
    /**
     * Profile with retry logic and exponential backoff.
     *
    ...
    private function extractYaml(string $response): string
    {
        ...
        // Case 1: YAML inside a markdown code block
        if (preg_match('/```(?:yaml)?\s*(campaign:.*?)```/s', $response, $matches)) {
            return trim($matches[1]);
        }

        // Case 2: Raw YAML (starts with "campaign:")
    ...
    /**
     * Validates the profile's YAML structure.
     *
     * @throws \RuntimeException If the YAML is invalid or incomplete
     */
    ...
    {
        $patterns = [
            // Plain-text email (e.g. user@example.com)
