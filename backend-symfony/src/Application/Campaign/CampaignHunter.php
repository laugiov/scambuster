...
final readonly class CampaignHunter
{
    ...
    private function huntRule(CampaignRule $rule): array
    {
        ...
        // to satisfy the constraint: true_pos + false_pos = hits_count
    ...
    /**
     * Validates hits (true positives vs false positives).
     *
    ...
    private function validateHits(array $hits): array
    {
        ...
        foreach ($hits as $hit) {
            ...
            if (!$messageId) {
                // Missing message ID = false positive
                $falsePos++;
            ...
            if ($message === null) {
                // Message not found = false positive
                $falsePos++;
            ...
            // Heuristic: score_risk >= 30 = true positive
    ...
    /**
     * Computes the lead-time (first hit → campaign peak).
     *
     * @param array<int, array<string, mixed>> $hits SQL results sorted by ts_msg
     *
     * @return int|null Lead-time in seconds, null if insufficient
     */
    private function calculateLeadTime(array $hits): ?int
    {
        if (count($hits) < 5) {
            return null; // Not enough hits to compute a peak
        }
        ...
        if (!$peakTime instanceof \DateTimeImmutable) {
            // Fallback: use the last hit as the peak
     ...
     *
     * @return \DateTimeImmutable|null Peak timestamp
     */
