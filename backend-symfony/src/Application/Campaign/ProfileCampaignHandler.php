...
/**
 * Handler that profiles a campaign via LLM.
 *
...
final readonly class ProfileCampaignHandler
{
     ...
     *
     * @throws \RuntimeException If the campaign does not exist or does not have enough messages
     *
    ...
    public function handle(Uuid $campaignId, int $sampleSize = self::DEFAULT_SAMPLE_SIZE): array
    {
        ...
        // 3. Call the LLM profiler
        $result = $this->profiler->profile($messages);

        // 4. Store the profile in the campaign
