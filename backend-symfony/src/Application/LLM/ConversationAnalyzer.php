...
final class ConversationAnalyzer
{
    ...
    /** @var array<string, array{analysis: string, repetitions_detected: array<string>, strategic_suggestions: array<string>, tone_recommendation: string, instructions_for_llm: array<string, mixed>, director: ConversationDirectorBrief}> In-memory cache */
    private array $analysisCache = [];

    public function __construct(
        private readonly LLMClientInterface $llmClient,
        private readonly LoggerInterface $logger,
        // Nullable + last so existing construction sites (and tests) keep working; when wired it
     ...
     *   instructions_for_llm: array<string, mixed>, director: ConversationDirectorBrief
     * }
     */
    ...
    private function buildAnalysisPrompt(array $context, array $preparedMessages): string
    {
        ...
        // Operator-overridable director blocks (strategy + tone). Resolve via the shared provider
        // (DB → file → shipped default); when unwired, fall back to the byte-identical catalog
        // default so the assembled prompt is unchanged. The JSON output contract, the interpolated
        // conversation data, the anti-unmask / hostile-state rules and the multilingual rule stay
        // hardcoded below and are never editable.
        $directorStrategyDefault = PromptCatalog::defaultBody('conversation_director_strategy');
        $directorStrategy = $this->prompts !== null
            ? $this->prompts->resolve('conversation_director_strategy', [], $directorStrategyDefault, [])
            : $directorStrategyDefault;

        $directorToneDefault = PromptCatalog::defaultBody('conversation_director_tone');
        $directorTone = $this->prompts !== null
            ? $this->prompts->resolve('conversation_director_tone', [], $directorToneDefault, [])
            : $directorToneDefault;
...

{
  ...
  "director": {
    "already_obtained": ["short labels of intel the correspondent has ALREADY provided, e.g. 'postal address', 'registration number', 'phone', 'client references' — semantic, in ANY language"],
    "mark_state": "cooperative|stalling|suspicious|anti_bot_challenge|hostile|disengaged",
    "objective": "the single goal for THIS exchange, inferred from the actual scam mechanics (e.g. for a cold web-services pitch: 'get them to name a price, then request an invoice and the payment method')",
    "progress": "advancing|stalled|regressing",
    "next_move": "the ONE best tactical instruction for the reply this turn",
    "style_directive": "how to SHAPE this reply so it does not read like the last ones: pick a form that fits the moment, e.g. 'lead with a short reaction, then ONE question', 'answer only, ask nothing this turn', 'two terse lines', 'a single blunt question', 'think out loud briefly then commit'. Vary it from turn to turn; never default to greeting+thanks+question.",
    "should_continue": true,
    "stop_reason": "if should_continue is false, one short sentence why (e.g. 'mark accused us of being a bot and threatened to block')"
  }
}

DIRECTOR (fill the "director" object above):
- You are conducting this conversation, not just answering. Hold a theory of the whole exchange.
- already_obtained: list EVERY piece of intel the correspondent has already given, however phrased or in whatever language. The persona MUST NOT ask for any of these again — re-asking is the #1 way our bot gets unmasked.
- mark_state: read the correspondent's arc. If they mock us, call us a bot/script, demand we answer first, or threaten to block/report → they are hostile/anti_bot_challenge/disengaged.
{$directorStrategy}
- should_continue: set to false when the mark has clearly disengaged or is only demanding we prove we are human and there is no path to more intel — continuing would only burn the persona.
...
2. STRATEGIC ANALYSIS:
   ...
   A persona asking BIC/SWIFT 3 turns in a row after the scammer has explicitly
   said "I'll share later once we finalize" reads as a script stuck in a loop.
   Pivot to a different angle instead.
   ...
   → ALSO produce two STRUCTURED arrays in the instructions
   ...
   When the attacker uses a classic influence principle, the persona should
   mirror it: give the attacker what he expects emotionally while extracting
   one more IOC.
...
4. INSTRUCTIONS FOR THE GENERATOR LLM (STRUCTURED JSON format mandatory):
   ...
   "forbidden_iocs" (array of short tokens):
   ...
   "pivot_to_iocs" (array of short tokens):
...
PROMPT;
    ...
    private function parseAnalysisResponse(string $llmResponse): array
    {
        try {
            ...
            // Guarantee forbidden_iocs + pivot_to_iocs keys exist with safe
            // empty defaults so PromptBuilder reads them unconditionally.
            // Older LLM responses don't emit these fields → they default to
            // [] here.
            ...
            // Director brief: reasoned strategic state for this turn. Absent or
            // malformed → safe default (keep going, nothing obtained), so the
            // pipeline degrades to its prior behaviour rather than failing.
            $director = is_array($decoded['director'] ?? null)
                ? ConversationDirectorBrief::fromArray($decoded['director'])
                : ConversationDirectorBrief::default();
    ...
    private function generateGenericInstructions(): array
    {
        return [
            ...
            'instructions_for_llm' => [
                ...
                // Fallback path must expose the new fields with safe empty
                // defaults for shape consistency with the LLM path.
                // PromptBuilder reads these without isset/?? guards.
                'forbidden_iocs' => [],
                'pivot_to_iocs' => [],
            ],
            'director' => ConversationDirectorBrief::default(),
        ];
    }
