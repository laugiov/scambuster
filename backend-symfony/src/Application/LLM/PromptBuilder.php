...
final readonly class PromptBuilder
{
    public function __construct(
        ...
        // Patch-mode retry: when the validator returned a
        ...
        // Resolves the operator-overridable EDITABLE persona rules (voice/style) through the
        // DB/file override chain. Nullable for test construction (same idiom as
        // $conversationAnalyzer); when null the shipped editable rules are used directly.
        private ?PromptProvider $prompts = null,
    ) {
    }
    ...
    public function buildGeneratorPrompts(array $context, string $personaCode): array
    {
        ...
        // The CORE rules (safety / anti-unmask / language invariant) are always injected and are
        // NOT operator-editable. Only the EDITABLE subset (voice/style) is resolved through the
        // override chain (operator UI / file), falling back to the shipped editable rules. This
        // reorders the block to core-then-editable (editable last, per recency intent).
        // Note: the editable override lands AFTER core, so a hostile override can add text that
        // competes for recency with the prompt-only anti-unmask core rule. Channel/payment core
        // rules are backstopped deterministically by PolicyGuard/PaymentInstigationGuard; the
        // anti-unmask one is backstopped by the ReplyValidator security gate + the pre-activation
        // canary (operators are a trusted, config:write role) — not by a deterministic gate.
        $editableDefault = BasePromptRules::getEditableRules($detectedLanguage);
        $editableRules = $this->prompts !== null
            ? $this->prompts->resolve('persona_style_rules', [], $editableDefault, [])
            : $editableDefault;
        $systemPrompt .= "\n\n" . BasePromptRules::getCoreRules($detectedLanguage) . "\n" . $editableRules;
        ...
        // Pairs with the BasePromptRules behavioral rule on payment cues.
        //
        // At `payment_push` only, pick a scam-type-aware OBJECTIVE
        // template. Buckets are resolved from `$context['scam_type']['code']`
        // (DB-classified at intake, never derived from message text — keeps
        // the payment-instigation guard LLM-only). Every payment_push template
        ...
        // Payment-anchoring gate for money-related objective lines. The
        // flag carries PaymentInstigationGuard's own anchoring verdict
        // (evaluated once per generation by RetryCoordinator): telling
        // the persona to ask "where the money will go" while the scammer
        // never raised payment is exactly what that guard then blocks —
        // the contradiction shipped deterministic 3-strike fallbacks.
        // Absent flag fails closed (no money asks): always-safe guidance.
        // payment_push templates are deliberately NOT gated — that stage
        // means money is on the table and extraction guidance must not
        // weaken.
        $paymentAnchored = (bool) ($context['payment_topic_anchored'] ?? false);

        $this->logger->debug('[PromptBuilder] bucket resolved', [
            'conv_id' => $context['conv_id'] ?? 'unknown',
            'scam_code' => $scamCode,
            'bucket' => $bucket,
            'stage' => $stage,
            'payment_anchored' => $paymentAnchored,
        ]);

        // Director-driven OBJECTIVE (LLM-reasoned). When the director has inferred
        // an objective for this turn, it drives the persona — this replaces the
        // static scam-type OBJECTIVE table for every conversation the analyzer saw.
        // The table below survives only as a graceful fallback for turn 1 or when
        // the analyzer is unavailable/failed.
        $directorBrief = (($this->runAnalyzerOrNull($context, $personaCode, $messageCount) ?? [])['director'] ?? null);

        $fallbackObjective = match ($stage) {
            'first_contact' =>
            ...
            'follow_up' => $paymentAnchored
                ...
                : "Stage: follow-up. The relationship is forming. Ask ONE practical question (when you need to act, how the process works step by step, who exactly you are dealing with, how to verify their organization). Do NOT bring up money, payment, or transfers — the correspondent has not raised them yet.\n",
            ...
            default => $paymentAnchored
                ? "Ask ONE concrete question to advance the conversation toward payment details.\n"
                : "Ask ONE concrete question to advance the conversation (timeline, process, identity, verification). Do NOT bring up money, payment, or transfers — the correspondent has not raised them yet.\n",
        };

        // The director's objective wins when it reasoned one; otherwise fall back
        // to the stage/scam-type guidance above (turn 1, or analyzer unavailable).
        if ($directorBrief instanceof ConversationDirectorBrief && $directorBrief->objective !== '') {
            $userPrompt .= "Your objective for this exchange: {$directorBrief->objective}\n";

            if ($directorBrief->nextMove !== '') {
                $userPrompt .= "Your single next move in this message: {$directorBrief->nextMove}\n";
            }
        } else {
            $userPrompt .= $fallbackObjective;
        }
        ...
        // Preventive instruction against signing.
        ...
        // Patch-mode: when the validator returned a structured
        ...
        // Conditional PRIORITY OVERRIDE section, placed
        ...
        // → prompt is unchanged in that case (dominant path).
     ...
     *
     * @param array<string, mixed>|null $context Optional
    ...
    public function buildValidatorPrompts(string $generatedText, string $personaCode, ?array $context = null): array
    {
        ...
        // System prompt may include the correction-field
...
PROMPT;
        ...
        // Append context block + identity-coherence check
    ...
    /**
     * Render the patch-mode block for a generator retry when
...
PROMPT;
    ...
    /**
     * Render the conversational context + identity-coherence
    ...
    private function buildValidatorContextBlock(array $context): string
    {
        ...
        // When the runtime knows the honeypot mailbox the persona
...
PROMPT;
    ...
    private function buildVarietySection(array $context, string $personaCode, int $messageCount): string
    {
        ...
        // Anti-repetition is now LLM-reasoned: the director lists the intel the
        // correspondent has ALREADY provided (semantic, in any language), and we
        // forbid re-asking any of it. This replaces the previous regex extraction
        // of "?"-shaped sentences, which could not catch reformulations or work
        // across long threads. Skipped when the analyzer is unavailable.
        $brief = $analysis['director'] ?? null;

        // The director also chooses the SHAPE of this turn's reply so consecutive
        // messages do not share the greeting -> thanks -> long-question mould that
        // reads as a template. Structural variety, not just word choice.
        if ($brief instanceof ConversationDirectorBrief && $brief->styleDirective !== '') {
            $baseVariety .= "Shape this reply as follows (structure, not exact words): {$brief->styleDirective}\n"
                . "Do not reuse the same structure as your previous messages.\n\n";
        }

        if ($brief instanceof ConversationDirectorBrief && $brief->alreadyObtained !== []) {
            $list = '';

            foreach ($brief->alreadyObtained as $item) {
                $list .= "  - {$item}\n";
            }

            $baseVariety .= "You have ALREADY obtained the following from the operator:\n"
                . $list
                . "Do NOT ask for any of these again — re-asking reads as robotic and is the #1 way to get unmasked as a bot. If you still need to move forward, ask for something NEW or take the next concrete step instead.\n\n";
        }
    ...
    /**
     * Invoke ConversationAnalyzer once per reply build,
     ...
     *   director: \App\Application\LLM\Director\ConversationDirectorBrief
     * }|null
     */
    ...
    /**
     * Build the conditional "## PRIORITY OVERRIDE" block
     ...
     * unchanged in that case (regression guarantee for ~99.75 % of replies
     * that don't hit the deferral path).
     *
     * @param array<string, mixed> $context
     */
...
DO NOT ask for these again in this message — re-asking would read as robotic and trigger bot detection.
