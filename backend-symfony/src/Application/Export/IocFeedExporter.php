...
final readonly class IocFeedExporter
{
    ...
    private function toCsv(array $records): string
    {
        ...
        // Column headers are constants — no neutralization needed.
        ...
        foreach ($records as $record) {
            ...
            foreach (self::COLUMNS as $column) {
                ...
                $line[] = $value === null ? '' : self::neutralizeCsvInjection((string) $value);
            }
     ...
     *
     * IOC values are scammer-controlled (e.g. an email Subject becomes a
     * `subject` IOC). A cell whose value begins with `= + - @`, TAB or CR is
     * interpreted as a formula by Excel / LibreOffice / Google Sheets when the
     * exported file is opened. Prefixing such a value with a single apostrophe
     * forces the spreadsheet to treat the whole cell as text. Values that do not
     * begin with a trigger are returned unchanged; RFC-4180 quoting is still left
     * to fputcsv.
     */
    private static function neutralizeCsvInjection(string $value): string
    {
        if ($value === '') {
            return $value;
        }

        // Look past leading whitespace: a spreadsheet may not evaluate " =1+1"
        // as written, but a pipeline stage that trims the cell first would
        // re-expose the formula. Prefix if the first non-whitespace character is
        // a formula lead (= + - @). A raw TAB/CR as the very first byte is itself
        // a trigger in some importers, so it is caught regardless.
        $lead = ltrim($value, " \t\r\n\x0B\x0C");

        if (($lead !== '' && str_contains('=+-@', $lead[0])) || str_contains("\t\r", $value[0])) {
            return "'" . $value;
        }
