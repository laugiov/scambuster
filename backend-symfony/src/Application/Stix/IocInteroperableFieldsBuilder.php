...
final class IocInteroperableFieldsBuilder
{
    ...
    {
        // Control-character stripping happens in stringOrNull() (the sink every
        // free-text field passes through); here we only collapse whitespace and
        // cap the length of the excerpt.
    ...
    private static function stringOrNull(mixed $value): ?string
    {
        ...
        // Defense at the sink: strip non-whitespace C0 control characters and DEL
        // from EVERY free-text field that reaches an exported description/label,
        // not only the excerpt. These bytes enable terminal/log-control injection
        // (ESC/BEL/NUL) and rendering ambiguity in a consumer such as OpenCTI.
        // Byte-oriented (no /u) so it can never corrupt valid UTF-8 multibyte
        // content — those bytes never occur inside a valid UTF-8 sequence.
        // C1 (U+0080-U+009F) is out of scope; see spec D3.
        $clean = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', '', $value) ?? $value;
        $trimmed = trim($clean);
