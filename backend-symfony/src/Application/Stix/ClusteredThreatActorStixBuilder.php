...
final readonly class ClusteredThreatActorStixBuilder
{
    ...
    private TtpAttackPatternBuilder $ttpBuilder;

    public function __construct()
    {
        $this->actorBuilder = new ThreatActorStixBuilder();
        $this->ttpBuilder = new TtpAttackPatternBuilder();
    }
    ...
    public function buildBundle(array $clusterData): array
    {
        ...
        // Scammer-side TTP layer: SB-T* attack-patterns for the cluster's confirmed
        // TTPs plus their uses-relationships and sightings. Additive to the MITRE
        // scam-type attack-patterns above; assembled from clusterData['ttps'] so
        // every cluster export surface (HTTP, TAXII, console) stays identical.
        /** @var list<array<string, mixed>> $ttps */
        $ttps = \is_array($clusterData['ttps'] ?? null) ? $clusterData['ttps'] : [];
        $clusterIdForTtps = \is_string($clusterData['cluster_id'] ?? null) ? $clusterData['cluster_id'] : '';
        $now = (new \DateTimeImmutable())->format('Y-m-d\TH:i:s.v\Z');

        foreach ($this->ttpBuilder->buildClusterTtpObjects($ttps, $actorId, $clusterIdForTtps, $now) as $ttpObject) {
            $objects[] = $ttpObject;
        }
    ...
    /**
     * Build embeddable threat-actor objects for inclusion
    ...
    private function buildThreatActor(array $clusterData): array
    {
        ...
        $actor = [
            ...
            'extensions' => [
                ...
                ScambusterStixExtensions::ACTOR_ID => ScambusterStixExtensions::wrap('x_scambuster_actor', [
                    'schema_version' => '2.0',
