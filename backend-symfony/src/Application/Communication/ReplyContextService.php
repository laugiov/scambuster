...
class ReplyContextService
{
    ...
    public function getConversationContext(string $convId, int $messageLimit = 5): ?array
    {
        ...
        $extractedIocs = $this->fetchExtractedIocs($convId);
        ...
        return [
            ...
            'persona' => $personaCode,
            // Free-text tone descriptor consumed by PolicyGuardConfig to
            // give terse archetypes a length floor their style can meet.
    ...
    /**
     * Map the conversation's ObservedIoc entities to the compact
     * type/value/category shape ConversationAnalyzer expects.
     *
     * IOC context is enrichment, not a hard dependency: on load failure
     * the reply still generates, but the failure is logged at WARNING —
     * a silent DEBUG swallow here once hid a TypeError that stripped IOC
     * context from every generation.
     *
     * @return array<int, array<string, mixed>>
     */
    private function fetchExtractedIocs(string $convId): array
    {
        ...
        try {
            ...
            return array_map(static function (\App\Domain\Communication\ObservedIoc $ioc): array {
                ...
                return [
                    ...
                    // Prefer the category stored at extraction time (the
                    // value the rest of the system persists); fall back to
                    // deriving it from the type when absent.
                    'category' => \is_string($context['category'] ?? null)
                        ? $context['category']
                        : \App\Domain\Communication\IocCategory::classify($type),
                ];
            }, $iocs);
        } catch (\Throwable $e) {
            $this->logger->warning('[ReplyContextService] Failed to fetch IOCs', [
                'conv_id' => $convId,
                'error' => $e->getMessage(),
            ]);
