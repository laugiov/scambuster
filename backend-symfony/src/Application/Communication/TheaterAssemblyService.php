...
/**
 * Composes the Live Bait Theater response in ONE round-trip.
 *
 * Returns meta + messages + iocs (deduplicated by value_norm, first-message
 * wins), plus revelation_context (LEFT JOIN on `ioc_context`) and
 * `human_factor` aggregates.
 *
 * Hard rules:
 ...
 *   excluded as well (orphan IOC rule).
...
final readonly class TheaterAssemblyService
{
    ...
    public function __construct(
        ...
        private TtpQueryService $ttpQuery,
    ) {
    }
     ...
     *   ttps_by_msg: list<array<string, mixed>>,
    ...
    public function assemble(Conversation $conv): array
    {
        ...
        // Enrich each IOC with its revelation_context
        // (LEFT JOIN on ioc_context). Orphan stimulus_msg_id validated
        // against the conv's message set.
        $iocsByMsg = $this->enrichWithRevelationContext($iocsByMsg, $messageIdSet);

        // Confirmed scammer TTPs per message, scoped to the same message set as
        // the IOCs (soft-delete / orphan / truncation window all shared).
        $ttpsByMsg = $this->buildConfirmedTtpsByMsg($conv->getConvId(), $messageIdSet);
        ...
        $meta = [
            ...
            // Human-readable persona label for display in
            ...
            // Count of IOCs in the Actionable tier (excludes
        ...
        return [
            ...
            'ttps_by_msg' => $ttpsByMsg,
            'human_factor' => $humanFactor,
        ];
    }

    /**
     * Confirmed TTP observations for the conversation, keyed by msg_id, scoped to
     * the same message set as iocs_by_msg (soft-deleted / orphan / out-of-
     * truncation-window messages are excluded via $messageIdSet). Review-status
     * rows are excluded from this showcase view. Evidence stays OFFSETS-ONLY
     * (UTF-8 code-point [start, end) into subject + "\n\n" + body_text); the
     * verbatim quote is never serialized.
     *
     * @param array<string, true> $messageIdSet
     *
     * @return list<array<string, mixed>>
     */
    private function buildConfirmedTtpsByMsg(string $convId, array $messageIdSet): array
    {
        $observations = $this->ttpQuery->conversationTtps($convId)['observations'];

        $out = [];

        foreach ($observations as $obs) {
            if ('confirmed' !== ($obs['status'] ?? null)) {
                continue;
            }

            $msgId = \is_string($obs['msg_id'] ?? null) ? $obs['msg_id'] : '';

            if ('' === $msgId || !isset($messageIdSet[$msgId])) {
                continue;
            }

            $out[] = [
                'msg_id' => $msgId,
                'ttp_code' => \is_string($obs['ttp_code'] ?? null) ? $obs['ttp_code'] : '',
                'ttp_label' => \is_string($obs['ttp_label'] ?? null) ? $obs['ttp_label'] : '',
                'phase' => \is_string($obs['phase'] ?? null) ? $obs['phase'] : '',
                'confidence' => \is_numeric($obs['confidence'] ?? null) ? (float) $obs['confidence'] : 0.0,
                'status' => 'confirmed',
                'evidence_start' => \is_int($obs['evidence_start'] ?? null) ? $obs['evidence_start'] : null,
                'evidence_end' => \is_int($obs['evidence_end'] ?? null) ? $obs['evidence_end'] : null,
            ];
        }
    ...
    /**
     * Single SQL fetch on ioc_context for all the IOCs
     ...
     */
    /**
     * Count IOCs in the Actionable tier. The Context tier
    ...
    {
        // Delegate to the shared policy so the List + Detail
    ...
    private function serializeAndDedupIocs(array $iocs, array $messageIdSet): array
    {
        ...
        foreach ($iocs as $ioc) {
            ...
            $out[] = [
                ...
                // Default; enrichWithRevelationContext() fills this from ioc_context.
