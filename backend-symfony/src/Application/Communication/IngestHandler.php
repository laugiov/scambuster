...
class IngestHandler
{
    ...
    public function ingest(IngestRawRequestDto $dto): array
    {
        ...
        // 1. Parse the raw email first, so the recipient address can drive
        // account resolution when no explicit account_id is supplied.
        ...
        $recipientEmails = $this->extractRecipientCandidates($parsed);

        // 2. Resolve reference entities. The account comes from account_id when
        // provided, otherwise from the message recipient — this lets an operator
        // plug in a new mailbox by configuration alone (no code change).
        if ($this->referenceResolver instanceof \App\Application\Communication\EntityReferenceResolver) {
            $refs = $this->referenceResolver->resolve($dto->account_id, $dto->channel ?? 'email', $recipientEmails);
            ...
            $account = $this->resolveAccountInline($dto->account_id, $recipientEmails);
        ...
        // 6a. Backend-side parser fallback for attachments.
        ...
        // 7b. Link the mail's attachments' sha256 into the IOC
    ...
    /**
     * Link the mail's attachments' sha256 hashes into the IOC
     ...
     * the honeypot guards, RuntimeException from a missing message, or any
    ...
    private function linkAttachmentsAsIocs(?array $attachmentDataList, string $msgId, \DateTimeImmutable $now): void
    {
        ...
        foreach ($attachmentDataList as $attData) {
            ...
            try {
                ...
                // Outgoing-message guard or honeypot filter.
                // Expected on outgoing messages — silent no-op, log at DEBUG.
                $this->logger->debug('[IngestHandler] Guard blocked attachment IOC linkage', [
                    'msg_id' => $msgId,
                    'filename' => $filename,
                    'reason' => $e->getMessage(),
                ]);
    ...
    /**
     * Extract candidate recipient addresses from a parsed message, ordered by
     * how reliably they identify the receiving mailbox: delivery headers first
     * (Delivered-To, X-Original-To), then To and Cc.
     *
     * @param array<string, mixed> $parsed
     *
     * @return list<string> Lowercased, de-duplicated bare email addresses
     */
    private function extractRecipientCandidates(array $parsed): array
    {
        $headers = \is_array($parsed['headers'] ?? null) ? $parsed['headers'] : [];

        $sources = [
            $headers['delivered-to'] ?? null,
            $headers['x-original-to'] ?? null,
            $parsed['to'] ?? ($headers['to'] ?? null),
            $headers['cc'] ?? null,
        ];
        ...
        foreach ($sources as $source) {
            ...
            if (preg_match_all('/[a-z0-9._%+\-]+@[a-z0-9.\-]+\.[a-z]{2,}/i', $source, $matches) > 0) {
    ...
    /**
     * Legacy account resolution used only when the EntityReferenceResolver is
     * not injected (older test setups). Mirrors the resolver's precedence:
     * explicit id first, then the recipient addresses.
     *
     * @param list<string> $recipientEmails
     */
    private function resolveAccountInline(string $accountId, array $recipientEmails): ?\App\Domain\Communication\MailAccount
    {
        $accountId = trim($accountId);
        $repo = $this->em->getRepository(\App\Domain\Communication\MailAccount::class);
        ...
        foreach ($recipientEmails as $email) {
            $account = $repo->findOneBy(['emailAddress' => $email, 'isActive' => true]);
