...
class IngestPostProcessor
{
    ...
    /**
     * Technical mailbox local-parts that mean "automated mail,
    ...
    public function __construct(
        ...
        private readonly ?TtpHandler $ttpHandler = null,
        private readonly string $operatorTestSenders = '',
    ) {
    }

    /**
     * Detect automated mail patterns BEFORE the
    ...
    public function processAfterIngest(Message $message, Conversation $conversation, string $detectedLang): void
    {
        ...
        // Pre-filter automated mail (DMARC, noreply, postmaster,
        ...
        // — /risk endpoint + n8n Decision Gate — refuses to reply
        ...
        if ($preFilterMatch !== null) {
            ...
            // Persist the pre-filter decision as a structured
            // marker on the message itself. The /risk endpoint
        ...
        $this->extractTtps($msgId);
    }

    /**
     * Tag the inbound message with scammer TTPs from the closed taxonomy.
     *
     * Runs in-process right after ingestion (the same hook the classifier and
     * injection detector use) so no external workflow is needed. Optional
     * dependency + fail-safe: a disabled module or an extraction failure never
     * affects ingestion; messages missed here are picked up by the backfill
     * command.
     */
    private function extractTtps(string $msgId): void
    {
        if (!$this->ttpHandler instanceof TtpHandler) {
            return;
        }

        try {
            $result = $this->ttpHandler->extractForMessage($msgId);
            $this->logger->info('[IngestPostProcessor] TTP extraction complete', [
                'msg_id' => $msgId,
                'ttps_found' => $result['ttps_found'],
                'persisted' => $result['persisted'],
            ]);
        } catch (TtpExtractionDisabledException) {
            // Module switched off — nothing to do, and nothing to log per message.
        } catch (\Throwable $e) {
            $this->logger->warning('[IngestPostProcessor] TTP extraction failed (non-blocking)', [
                'msg_id' => $msgId,
                'error' => $e->getMessage(),
            ]);
        }
