...
class ScamClassificationHandler
{
    public function __construct(
        ...
        // Optional, nullable to preserve callsites that
    ...
    public function classifyConversation(string $convId): ClassificationResult
    {
        ...
        // Threshold lowered from 0.75 to 0.55 to accept
    ...
    /**
     * Emit SCAM_CLASSIFIED audit_log row so UNKNOWN-rate
     ...
     * Get conversation messages for classification
     *
     * Find non-deleted conversations created within the trailing $days
     * whose current scam type is UNKNOWN — the scope for a bounded
     * re-classification backfill of the recent past.
     *
     * @return list<string> conversation ids, newest first
     */
    public function findRecentUnknownConversationIds(int $days, ?int $limit = null): array
    {
        ...
        /** @var array<int, array{convId: mixed}> $rows */
        $rows = $qb->getQuery()->getResult();

        // Normalize to plain strings: the uuid column can hydrate as a
        // value object depending on the DBAL type, and callers compare
        // against Conversation::getConvId() (a string).
    ...
    /**
     * Re-run the classifier for a conversation WITHOUT persisting anything
     * — the read-only preview a backfill uses to report proposed labels
     * before an operator commits to applying them.
     *
     * Returns null when the conversation has no messages to classify.
     */
    public function previewClassifyConversation(string $convId): ?ClassificationResult
    {
        $messages = $this->getConversationMessages($convId);
        ...
        return $this->scamClassifier->classify($messages);
    }
