...
final readonly class TotpLoginController
{
    public function __construct(
        ...
        // Replaces the custom RFC 6238 implementation with scheb
        private ?TotpAuthenticatorInterface $totpAuthenticator = null,
        // Rate limiting on 2FA endpoint (reuses login_ip limiter)
        private ?RateLimiterFactory $loginIpLimiter = null,
        // TOTP replay protection via cache
        private ?\Psr\Cache\CacheItemPoolInterface $totpReplayCache = null,
    ) {
    }
    public function __invoke(Request $request): JsonResponse
    {
        // Rate limit check (same limiter as login endpoint)
        ...
        // TOTP replay protection: reject codes already used within 90s window
        ...
        // Delegate verification to scheb/2fa-bundle if available,
        ...
        // Mark TOTP code as used (90s TTL covers the 30s window + clock skew)
        if ($this->totpReplayCache instanceof \Psr\Cache\CacheItemPoolInterface) {
            $replayCacheKey = 'totp_used_' . md5($email . ':' . $code);
            $cacheItem = $this->totpReplayCache->getItem($replayCacheKey);
            $cacheItem->set(true);
