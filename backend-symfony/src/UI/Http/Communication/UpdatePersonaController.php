...
final class UpdatePersonaController extends AbstractController
{
    ...
    public function __invoke(Request $request, string $personaCode): JsonResponse
    {
        ...
        //    `reset_stats` is a control flag (not a persona field): when true, the persona's
        //    bandit stats are cleared so a prompt change doesn't bias exploration.
        $allowedFields = ['persona_label', 'persona_tone', 'system_prompt', 'reset_stats'];
        ...
        // 4. At least one field (or a stats reset) must be provided
        ...
        $resetStats = ($body['reset_stats'] ?? false) === true;

        if ($label === null && $tone === null && $prompt === null && !$resetStats) {
            return new JsonResponse([
                'success' => false,
        ...
        // 6a. Audit the field change. Editing a persona (especially its system prompt)
        //     changes how the LLM behaves, so every mutation must leave a trail.
        $changedFields = [];

        if ($label !== null) {
            $changedFields[] = 'persona_label';
        }

        if ($tone !== null) {
            $changedFields[] = 'persona_tone';
        }
        ...
        // 6b. Reset the persona's bandit stats if requested (prompt changed → old reward
        //     no longer reflects behaviour → keeping it would bias exploration).
        if ($resetStats) {
            $removed = $this->personaManager->resetStats($persona);
            $this->auditLogger->log(
                eventType: AuditEventType::CONFIG_CHANGED,
                actorId: $this->getUser()?->getUserIdentifier() ?? 'unknown',
                action: 'persona.reset_stats',
                outcome: 'success',
                resourceType: 'persona',
                resourceId: $persona->getPersonaCode(),
