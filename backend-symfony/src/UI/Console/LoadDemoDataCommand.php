...
{
    /**
     * Per-scam-type stimulus progression, indexed by conversation phase
     * [early, mid, late, closing]. A plausible, deterministic arc used only to
     * spread the demo's stimulus_type across a conversation instead of pinning a
     * single constant per scam genre (which collapsed the Stimulus x TTP matrix
     * to a relabelled scam-type facet). First-contact revelations are PASSIVE and
     * bypass this table entirely. These are illustrative seed values, not model
     * output.
     *
     * Seven closed values are used: PASSIVE, TRUST_BUILDING, DIRECT_REQUEST,
     * DOCUMENT_REQUEST, PAYMENT_INITIATION, URGENCY_PRESSURE, UNKNOWN.
     *
     * @var array<string, array{0: string, 1: string, 2: string, 3: string}>
     */
    private const STIMULUS_ARC = [
        'INVOICE_FRAUD' => ['DIRECT_REQUEST', 'DOCUMENT_REQUEST', 'PAYMENT_INITIATION', 'URGENCY_PRESSURE'],
        'CEO_FRAUD' => ['DIRECT_REQUEST', 'PAYMENT_INITIATION', 'URGENCY_PRESSURE', 'URGENCY_PRESSURE'],
        'ADVANCE_FEE_419' => ['DIRECT_REQUEST', 'DOCUMENT_REQUEST', 'PAYMENT_INITIATION', 'URGENCY_PRESSURE'],
        'LOTTERY' => ['DIRECT_REQUEST', 'DOCUMENT_REQUEST', 'PAYMENT_INITIATION', 'URGENCY_PRESSURE'],
        'ROMANCE' => ['TRUST_BUILDING', 'TRUST_BUILDING', 'PAYMENT_INITIATION', 'URGENCY_PRESSURE'],
        'CHARITY' => ['TRUST_BUILDING', 'DIRECT_REQUEST', 'PAYMENT_INITIATION', 'URGENCY_PRESSURE'],
        'INVESTMENT' => ['TRUST_BUILDING', 'DOCUMENT_REQUEST', 'PAYMENT_INITIATION', 'URGENCY_PRESSURE'],
        'TECH_SUPPORT' => ['URGENCY_PRESSURE', 'DIRECT_REQUEST', 'PAYMENT_INITIATION', 'UNKNOWN'],
        'JOB_OFFER' => ['DIRECT_REQUEST', 'DOCUMENT_REQUEST', 'PAYMENT_INITIATION', 'URGENCY_PRESSURE'],
        'PHISHING' => ['DIRECT_REQUEST', 'UNKNOWN', 'URGENCY_PRESSURE', 'URGENCY_PRESSURE'],
        'PHISH_CREDENTIALS' => ['DIRECT_REQUEST', 'UNKNOWN', 'URGENCY_PRESSURE', 'URGENCY_PRESSURE'],
        'PHISH_MALWARE' => ['DIRECT_REQUEST', 'UNKNOWN', 'URGENCY_PRESSURE', 'URGENCY_PRESSURE'],
        'COLD_SERVICE_SPAM' => ['DIRECT_REQUEST', 'UNKNOWN', 'DIRECT_REQUEST', 'URGENCY_PRESSURE'],
    ];

    /** @var array{0: string, 1: string, 2: string, 3: string} */
    private const STIMULUS_ARC_DEFAULT = ['DIRECT_REQUEST', 'DOCUMENT_REQUEST', 'PAYMENT_INITIATION', 'URGENCY_PRESSURE'];

    /** Provenance stamp written to ioc_context.enrichment_model for demo-seeded enrichment. */
    private const DEMO_ENRICHMENT_MODEL = 'demo-seed';
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        // Two labelled honeypot mailboxes so the Conversations "Mailbox"
        // filter and column have something to show. Each conversation is
        // routed to one by its `mailbox` slug ("personal" | "business").
        $mailboxes = $this->ensureDemoMailboxes();
        $accountId = $mailboxes['personal'];
        ...
        try {
            ...
            foreach ($conversations as $conv) {
                ...
                // Route the conversation to its honeypot mailbox (defaults
                // to personal for older dataset rows without a slug).
                $mailboxSlug = (string) ($conv['mailbox'] ?? 'personal');
                $convAccountId = $mailboxes[$mailboxSlug] ?? $mailboxes['personal'];
    ...
    /**
     * Ensure the two labelled honeypot mailboxes the demo routes
     * conversations to exist and are active, and return the slug -> id map.
     *
     * Idempotent (upsert on the fixed account ids): re-seeding refreshes
     * the label/email/active flag without creating duplicates. These are
     * distinct, deterministic demo accounts, independent of the
     * environment-specific accounts seeded by MailAccountFixtures.
     *
     * @return array{personal: string, business: string}
     */
    private function ensureDemoMailboxes(): array
    {
        ...
        $mailboxes = [
            'personal' => [
                ...
                'email' => 'helen.baiter@inbox.demo',
            ],
            'business' => [
                'id' => 'dcdcdcdc-0000-4000-8000-000000000002',
                'label' => 'Business Inbox',
                'email' => 'accounts@brightpath.demo',
            ],
        ];
        ...
        foreach ($mailboxes as $mb) {
            $this->connection->executeStatement(
                "INSERT INTO mail_account
                    (account_id, owner_id, protocol, endpoint, login_hash, oauth_scopes, is_active, created_at, updated_at, email_address, label)
                 VALUES
                    (:id, :owner, 'IMAP', 'imap.demo.local', :hash, :scopes, true, NOW(), NOW(), :email, :label)
                 ON CONFLICT (account_id) DO UPDATE
                    SET label = EXCLUDED.label, email_address = EXCLUDED.email_address, is_active = true, updated_at = NOW()",
                [
                    'id' => $mb['id'],
                    'owner' => $ownerId,
                    'hash' => 'demo-' . $mb['label'],
                    'scopes' => json_encode(['mail.read', 'mail.send'], JSON_THROW_ON_ERROR),
                    'email' => $mb['email'],
                    'label' => $mb['label'],
                ]
            );
        ...
        return [
            'personal' => $mailboxes['personal']['id'],
            'business' => $mailboxes['business']['id'],
        ];
    }
    ...
    /**
     * Derive a deterministic, per-message demo stimulus_type.
     *
     * A revelation with no preceding outbound (stimulus_msg_id NULL — first
     * contact) was volunteered unprompted, so it is PASSIVE. Otherwise the value
     * follows the scam type's arc, bucketed by the message's turn position within
     * the conversation: early (<=3), mid (4-5), late (6-7), closing (>=8). Pure
     * function of its inputs — no randomness — so a purge/reseed is reproducible.
     *
     * @param int $turn 1-based message position within the conversation
     */
    public static function deriveDemoStimulus(string $scamType, int $turn, bool $hasPrecedingOutbound): string
    {
        if (!$hasPrecedingOutbound) {
            return 'PASSIVE';
        }
        ...
        $arc = self::STIMULUS_ARC[$scamType] ?? self::STIMULUS_ARC_DEFAULT;
     ...
     * (except stimulus_type, which additionally varies per message; see
     * deriveDemoStimulus).
     */
    private function applyDemoSemanticEnrichment(SymfonyStyle $io): int
    {
        ...
        $rows = $this->connection->fetchAllAssociative(
            "SELECT ic.id, ic.obs_id, ic.scam_type_code, ic.revelation_turn, ic.total_turns,
                    ic.stimulus_msg_id, i.type AS ioc_type
             FROM ioc_context ic
        ...
        foreach ($rows as $row) {
            ...
            // Stimulus — deterministic per message: a revelation volunteered with
            // no preceding outbound is PASSIVE (unsolicited), otherwise the value
            // follows a plausible per-scam progression keyed to the message's turn
            // position. No randomness, and stimulus_msg_id (set by the structural
            // step to the real preceding outbound, NULL on first contact) is left
            // intact so the causality link and the stimulus stay consistent.
            $hasPrecedingOutbound = ($row['stimulus_msg_id'] ?? null) !== null;
            $stimulus = self::deriveDemoStimulus($scamType, $turn, $hasPrecedingOutbound);
            ...
            $this->connection->executeStatement(
                ...
                [
                    ...
                    'stimulus' => $stimulus,
                    'urgency' => $urgency,
                    // Provenance stamp: marks every demo-enriched row as a seeded
                    // approximation, distinguishable from real LLM enrichment
                    // (which carries the actual model name, or NULL when unset).
                    'enrichmentModel' => self::DEMO_ENRICHMENT_MODEL,
                    'langSwitch' => 'false',
