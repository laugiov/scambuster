...
/**
 * Add a new mail account with optional per-account SMTP credentials.
 *
 * Usage:
 ...
 *
 * Outputs the new account_id (UUID) on stdout. The ingest pipeline matches
 * inbound mail to this account by its recipient address, so registering the
 * account is enough to onboard a mailbox — no workflow change is required.
 */
#[AsCommand(
    name: 'app:mail-account:add',
    description: 'Add a mail account with optional per-account SMTP DSN (encrypted at rest)',
)]
