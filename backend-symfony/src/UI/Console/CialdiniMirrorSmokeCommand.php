...
/**
 * Smoke harness — drives the FULL reply pipeline end-to-end and
 ...
 *   `13_CrossROMANCE_secrecy_EN.eml`        → expect Secrecy + romance bucket
 *
 * Multi-turn .json fixtures (MT*.json) follow the same format as
 * ReplyObjectiveSmokeCommand — see its docblock for shape.
 */
#[AsCommand(name: 'scambuster:smoke:cialdini-mirror', description: 'Smoke harness — drive reply pipeline, capture Cialdini-mirror detection in strategic_suggestions.')]
final class CialdiniMirrorSmokeCommand extends Command
{
    ...
    {
        $this
            ->addOption('fixtures-dir', null, InputOption::VALUE_OPTIONAL, 'Directory holding .eml or .json fixtures', 'tests/Smoke/CialdiniMirrorFixtures')
            ->addOption('output-dir', null, InputOption::VALUE_OPTIONAL, 'Directory to write per-test .md artifacts', 'var/smoke/cialdini-mirror')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('Smoke run (Cialdini mirror detection)');
    ...
    private function buildContext(string $scamCode, string $language, string $body, string $basename): array
    {
        ...
        // as the sibling smoke harness). Stubs are deliberately vague so they do NOT
    ...
    private function dumpSingleTurnArtifact(string $path, array $data): void
    {
        ...
        $md .= "## Verdict (filled during smoke-test.md review)\n\n"
            ...
            . "- (c) humanness: ?\n"
            . "- (d) language fidelity: ?\n"
            . "- (e) no regression (payment/out-of-band/anti-repetition guards): ?\n\n"
            . "**Overall verdict**: ?\n";
    ...
    private function dumpMultiTurnArtifact(string $path, array $data): void
    {
        ...
        $md .= "## Verdict (filled during smoke-test.md review)\n\n"
            ...
            . "- Language fidelity: ?\n"
            . "- Guardrail invariants (no out-of-band channel / no payment instigation / no repeated questions): ?\n\n"
            . "**Overall verdict**: ?\n";
