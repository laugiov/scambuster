...
/**
 * Stratified sampler for human-factor evaluation.
 *
 ...
 *
 * Output:
 *   - {out_dir}/test-set.csv      (100 rows, fields: obs_id, indicator_id, msg_id, conv_id, current_*)
 *   - {out_dir}/train-set.csv     (50  rows, same shape)
 *   - {out_dir}/test-set.sha256
 *   - {out_dir}/train-set.sha256
 *
 ...
 *   test-set.csv  = 1f32bb406cdfd80705d2c8d6b909572907a98058cb53350a76d9fe5030c694af
 *   train-set.csv = dca0021891c624326de49c71b050bc3bac0bbe20285d9af5cde5d1356a3e8493
 *
...
#[AsCommand(
    ...
    description: 'Stratified sample of 150 IOC contexts (train 50 / test 100) for human-factor calibration evaluation',
)]
final class EvalSampleTestSetCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        mt_srand($seed);
        $io->title('Stratified test-set sampler');
        ...
        // Assert train/test isolation (no obs_id overlap)
        ...
        $testPath = rtrim($outDir, '/') . '/test-set.csv';
        $trainPath = rtrim($outDir, '/') . '/train-set.csv';
        ...
        file_put_contents($testPath . '.sha256', "{$testSha}  test-set.csv\n");
        file_put_contents($trainPath . '.sha256', "{$trainSha}  train-set.csv\n");
