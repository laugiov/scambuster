...
class ConversationGenerator
{
    ...
    private const MIN_TURNS = 2;      // Minimum 2 turns (1 scammer + 1 victim)
    /** @phpstan-ignore-next-line Reserved for future use */
    private const MAX_TURNS = 15;     // Maximum 15 turns (30 messages) for performance
     ...
     *
     * @param ScamType $scamType     Scam type
     * @param Persona  $persona      Persona to use
     * @param Channel  $channel      Communication channel
     * @param int      $messageCount Number of messages (2-15)
     */
    public function generateConversation(
        ...
        for ($i = 0; $i < $counter; $i++) {
            // Support both formats: array with role/content OR plain string
        ...
        // IOC extraction via HTTP API (like the n8n workflow - works in prod)
        ...
        foreach ($messages as $message) {
            ...
            try {
                $this->logger->info('[IOC-HTTP-API] BEFORE extractIocsViaHttp()', [
                    'msg_id' => $msgIdStr,
                    'method' => 'llm',
                ]);

                // Use the HTTP API approach like the n8n workflow (works in prod)
        ...
        // via the /close API, which calculates rewards and updates the
        // epsilon-greedy stats.
...
PROMPT;
    ...
    /**
     * Returns the subject templates per scam type
     *
    ...
    /**
     * Extracts a message's IOCs via the HTTP API (like the n8n workflow)
     *
        ...
        try {
            ...
            $response = $this->httpClient->request(
                ...
                [
                    ...
                    'timeout' => 30,  // 30s timeout for the LLM call
                ]
            );
