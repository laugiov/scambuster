...
class IocContextTestFixtures extends Fixture implements DependentFixtureInterface, FixtureGroupInterface
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        // Find an inbound message with a conversation. The ORDER BY is REQUIRED, not
        // cosmetic: without it the pick is heap-order dependent, so these IOCs land on a
        // different conversation across fixture reloads and any test that assumes a given
        // conversation carries IOCs (MISP export on conv ...0001, the TAXII context-parity
        // test, etc.) flakes intermittently. Deterministically target the lowest conv_id's
        // earliest inbound message (conv ...0001), which the dependent tests expect.
        $inboundMsg = $conn->fetchAssociative(
            ...
            . ' ORDER BY m.conv_id ASC, m.ts_msg ASC, m.msg_id ASC'
            . ' LIMIT 1'
        );
