...
/**
 * Complete fixtures for Campaign Radar Phase 2-5.
 *
 * Scenarios covered:
 * - Shadow campaign with rules that are candidates for promotion
 * - Promoted campaign with active rules
 * - Archived campaign
 * - Rules with different PPV, hits, lead-time
 * - Messages detected per campaign (MessageCampaign)
 * - LLM profiles (YAML) with different IoC types
 */
class CampaignFixtures extends Fixture implements DependentFixtureInterface
{
    public function load(ObjectManager $manager): void
    {
        ...
        if (empty($messages)) {
            // No messages, skip fixtures
            return;
        }

        // ===== Campaign 1: Shadow - Promotion candidate (PPV=0.9, hits=10) =====
        ...
        // Rule 1.1: Promotable (PPV=0.92, hits=12, lead_time=4h)
        ...
        $rule11->setLeadTimeSec(14400); // 4 hours
        $manager->persist($rule11);

        // Rule 1.2: Non-promotable (PPV=0.75, insufficient hits)
        ...
        // ===== Campaign 2: Promoted with an active rule in production =====
        ...
        // Rule 2.1: Promoted and active (excellent PPV=0.95, hits=50)
        ...
        $rule21->setLeadTimeSec(7200); // 2 hours
        $rule21->promote(); // Mark as promoted
        $manager->persist($rule21);

        // ===== Campaign 3: Shadow - Multiple IoC types for STIX testing =====
        ...
        // ===== Campaign 4: Archived (old campaign) =====
        ...
        // ===== MessageCampaign: Link messages to campaigns =====
        // Link a few messages to campaign1 to simulate detection
        if (count($messages) >= 3) {
    ...
    /**
     * PayPal phishing YAML profile with domains, URLs, emails.
     */
...
YAML;
    ...
    /**
     * Bank wire transfer scam YAML profile with IBAN, domains.
     */
...
YAML;
    ...
    /**
     * Malware delivery YAML profile with file hashes, C2 servers.
     */
