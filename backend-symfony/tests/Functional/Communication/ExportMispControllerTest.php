...
final class ExportMispControllerTest extends WebTestCase
{
    ...
    // ──────────────────────────────────────────────
    // EVENT-LEVEL SCAM TAXONOMY (multi-label + MISP galaxy)
    // ──────────────────────────────────────────────

    public function testMispEventCarriesMultiLabelScamTaxonomyTags(): void
    {
        // conv 0001 has IOCs → 200. Pin the FULL classification (primary UNKNOWN +
        // a ROMANCE secondary) so the assertions are deterministic and order-independent
        // — an earlier test may have committed a different primary onto this fixture.
        /** @var \Doctrine\DBAL\Connection $conn */
        $conn = static::getContainer()->get('doctrine.dbal.default_connection');
        $orig = $conn->fetchAssociative('SELECT scam_type_id, secondary_scam_types FROM conversation WHERE conv_id = ?', [self::CONV_OPEN]);
        self::assertIsArray($orig);
        $unknownId = $conn->fetchOne("SELECT scam_type_id FROM lkp_scam_type WHERE code = 'UNKNOWN'");

        try {
            $conn->executeStatement(
                'UPDATE conversation SET scam_type_id = :pid, secondary_scam_types = :sec::jsonb WHERE conv_id = :cid',
                ['pid' => $unknownId, 'sec' => json_encode([['code' => 'ROMANCE', 'confidence' => 0.6]]), 'cid' => self::CONV_OPEN],
            );
            ...
            $tagNames = array_column($data['Event']['Tag'] ?? [], 'name');

            // Primary (UNKNOWN → RSIT other, no galaxy) + secondary (ROMANCE → RSIT scam + ATT&CK galaxy).
            $this->assertContains('scambuster:scam-type="UNKNOWN"', $tagNames);
            $this->assertContains('rsit:fraud="other"', $tagNames);
            $this->assertContains('scambuster:scam-type="ROMANCE"', $tagNames);
            $this->assertContains('rsit:fraud="scam"', $tagNames);
            $this->assertContains('misp-galaxy:mitre-attack-pattern="Impersonation - T1656"', $tagNames);
        } finally {
            $conn->executeStatement(
