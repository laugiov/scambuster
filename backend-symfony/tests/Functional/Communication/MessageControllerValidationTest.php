...
final class MessageControllerValidationTest extends WebTestCase
{
    ...
    {
        // Regression: the outgoing-message guard must not break legitimate extraction on incoming messages.
        $this->client->request('POST', self::BASE_URL . '/' . self::MSG_INBOUND_1 . '/extract-iocs', [], [], [
            'HTTP_AUTHORIZATION' => 'Bearer fake-jwt',
            'CONTENT_TYPE' => 'application/json',
        ], json_encode(['method' => 'regex']));
        ...
        if ($statusCode === Response::HTTP_BAD_REQUEST) {
            $data = json_decode($this->client->getResponse()->getContent(), true);
            $this->assertIsArray($data);
            // Must NOT be the outgoing-message error
