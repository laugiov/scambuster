...
final class ConversationDtoMailAccountFieldsTest extends WebTestCase
{
    ...
    public function testListConversationsPopulatesLabelAndEmailWhenAccountHasThem(): void
    {
        $convId = $this->seedConversationWithLabeledMailbox(
            label: 'Tarrowby Holdings',
            email: 'admin@tarrowbyholdings.com',
        );
        ...
        $this->assertSame('Tarrowby Holdings', $found['account_label']);
        $this->assertSame('admin@tarrowbyholdings.com', $found['account_email']);
    }
    ...
    public function testGetConversationPopulatesLabelAndEmailWhenAccountHasThem(): void
    {
        ...
        $this->assertResponseIsSuccessful();
        $data = json_decode($this->client->getResponse()->getContent(), true);
        $this->assertSame('Calverton Partners', $data['account_label']);
        $this->assertSame('admin@calvertonpartners.com', $data['account_email']);
    }

    public function testGetConversationHandlesNullLabelGracefully(): void
    {
        // Email present but label null (legacy data: spec 050 added email_address before label rollout)
        $convId = $this->seedConversationWithLabeledMailbox(
            label: null,
