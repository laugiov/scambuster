...
/**
 * Theater endpoint functional contract.
 *
...
final class GetConversationTheaterControllerTest extends WebTestCase
{
    ...
    public function testResponseStructureForFixtureConversation_097S1(): void
    {
        ...
        $this->assertArrayHasKey('ttps_by_msg', $data);
        ...
        // persona_label is now part of meta (human-readable
        // label used by the Theater header). Key is always present; value
        // may be null for legacy conversations without a Persona FK.
        $this->assertArrayHasKey('persona_label', $meta, 'meta must contain persona_label');
        $this->assertTrue(
            $meta['persona_label'] === null || \is_string($meta['persona_label']),
            'meta.persona_label must be string|null',
        );
        // Actionable-tier IOC count exposed for cross-check.
        $this->assertArrayHasKey('iocs_count_actionable', $meta, 'meta must contain iocs_count_actionable');
    ...
    public function testTtpsByMsgEntriesHaveExpectedShape_154(): void
    {
        $seed = $this->pickConvAndInboundMsg();
        if (null === $seed) {
            $this->markTestSkipped('No fixture conversation with an inbound message available');
        }
        [$convId, $msgId] = $seed;
        $ttpId = $this->ttpId('SB-T001');
        ...
        try {
            ...
            $this->assertArrayHasKey('ttps_by_msg', $data);
            $this->assertNotEmpty($data['ttps_by_msg'], 'The seeded confirmed TTP must surface');

            foreach ($data['ttps_by_msg'] as $ttp) {
                foreach (['msg_id', 'ttp_code', 'ttp_label', 'phase', 'confidence', 'status', 'evidence_start', 'evidence_end'] as $key) {
                    $this->assertArrayHasKey($key, $ttp, "ttps_by_msg entry must contain {$key}");
                }
                $this->assertSame('confirmed', $ttp['status']);
                // Verbatim evidence text must never cross the wire.
                $this->assertArrayNotHasKey('evidence', $ttp);
            }
        } finally {
            $this->conn->executeStatement(
                'DELETE FROM ttp_observation WHERE msg_id = :m AND ttp_id = :t',
                ['m' => $msgId, 't' => $ttpId],
            );
        }
    }
    ...
    /**
     * Any non-soft-deleted conversation with an inbound message. Unlike the other
     * pick helpers this does NOT exclude the `00000000-%` fixture convs — the
     * functional fixture DB contains only those, and this test neither soft-deletes
     * nor mutates the conversation (it inserts one ttp_observation row and removes
     * it in `finally`), so there is no cross-test interference.
     *
     * @return array{0: string, 1: string}|null [conv_id, inbound msg_id]
     */
    private function pickConvAndInboundMsg(): ?array
    {
        ...
        return [(string) $row['conv_id'], (string) $row['msg_id']];
    }

    private function ttpId(string $code): int
    {
        $id = $this->conn->fetchOne('SELECT ttp_id FROM lkp_ttp WHERE code = :code', ['code' => $code]);
        $this->assertIsNumeric($id, sprintf('Taxonomy code %s must exist in lkp_ttp', $code));
