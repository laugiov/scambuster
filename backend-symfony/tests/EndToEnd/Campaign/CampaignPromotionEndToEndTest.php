...
/**
 * End-to-End tests for Campaign Promotion & STIX Export.
 *
 * Tests the full workflow via REST API:
 * 1. GET /api/v1/campaign/candidates - List promotion candidates
 * 2. POST /api/v1/campaign/rule/{ruleId}/promote - Manual promotion
 * 3. POST /api/v1/campaign/{campaignId}/export/stix - STIX 2.1 export
 */
class CampaignPromotionEndToEndTest extends WebTestCase
{
    ...
    public function testGetPromotionCandidatesReturnsValidStructure(): void
    {
        ...
        // If at least one candidate, verify structure
    ...
    public function testPromoteCampaignRuleWorkflow(): void
    {
        ...
        // Step 1: Verify rule appears in candidates
        $client->request(
            'GET',
            '/api/v1/campaign/candidates',
            [],
        ...
        // Verify STIX file exists and is valid
    ...
    public function testPromoteRuleWithLowPPVReturns400(): void
    {
        ...
        // Create a rule with too-low PPV
        $em = $client->getContainer()->get('doctrine')->getManager();
    ...
    public function testPromoteRuleWithLowHitsReturns400(): void
    {
        ...
        // Create a rule with insufficient hits
        $em = $client->getContainer()->get('doctrine')->getManager();
        ...
        $rule->setCompiledData(['sql' => 'SELECT 1', 'params' => []]);
        $rule->enable();
        $rule->updateMetrics(3, 3, 0); // PPV=1.0 but hits=3 < 5
        $em->persist($rule);
        $em->flush();
    ...
    public function testExportSTIXWithoutPII(): void
    {
        ...
        // Create campaign with a profile containing PII (will be filtered)
        $em = $client->getContainer()->get('doctrine')->getManager();
