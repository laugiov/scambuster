...
/**
 * Tests unitaires RENFORCÉS pour CampaignProfiler
 *
 * Couvre:
 * - Edge cases de validation (messages multiples, limites)
 * - Retry logic complet (timeout, erreurs, backoff)
 * - Cache behavior (hit/miss, TTL, invalidation)
 * - Validation YAML (structure, types, clés requises)
 * - PII detection (emails, téléphones, IBANs)
 * - Extraction YAML (markdown, formats multiples)
 * - Scénarios réalistes (PayPal, banques, multi-tentatives)
 */
final class CampaignProfilerEnhancedTest extends TestCase
{
    ...
    {
        // Créer 15 messages (au-delà de la limite)
        $messages = [];
        for ($i = 0; $i < 15; $i++) {
    ...
    {
        // Même ensemble de messages → même clé de cache
        ...
        // Second call avec mêmes messages devrait utiliser même cache key
        $this->profiler->profile($messages2);

        // Vérifié par le mock cache (2 appels get)
