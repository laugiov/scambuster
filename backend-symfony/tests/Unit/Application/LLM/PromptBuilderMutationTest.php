...
final class PromptBuilderMutationTest extends TestCase
{
    ...
    public function test_objective_word_count_defaults_20_150(): void
    {
        $prompts = $this->builder->buildGeneratorPrompts($this->baseContext(), 'generic_user');
        $this->assertStringContainsString('20-150 words', $prompts['user']);
    }
    ...
    public function test_missing_iocs_listed_when_present(): void
    {
        ...
        $prompts = $this->builder->buildGeneratorPrompts($context, 'generic_user');
        // Replaced the weak "If natural" directive with a
    ...
    // ─── Stage-aware OBJECTIVE directive ──────────────

    /**
     * first_contact stage (1-2 messages, no payment kw).
     ...
     *
     */
    public function test_objective_first_contact_directive(): void
    {
        $context = $this->baseContext([
            'last_messages' => [
    ...
    /**
     * follow_up stage (3-5 messages, no payment keyword).
     * The OBJECTIVE must ask a practical question, not yet a specific IOC.
     */
    public function test_objective_follow_up_directive(): void
    {
    ...
    /**
     * payment_push stage (any message with payment keyword OR 6+ messages).
     * The OBJECTIVE must explicitly request a concrete IOC question.
     *
     * The concrete-IOC anchors are now scam-type-aware.
     ...
     * `PromptBuilderTest::test_uses_banking_template_for_*`.
     */
    public function test_objective_payment_push_directive(): void
    {
        ...
        $this->assertStringContainsString('phishing-style request', $prompts['user'], 'OBJECTIVE must announce the phishing_pull bucket for PHISHING scam_type');
        $this->assertStringContainsString('exact site', $prompts['user'], 'OBJECTIVE must list exact-site IOC anchor for phishing_pull');
        $this->assertStringContainsString('credential', $prompts['user'], 'OBJECTIVE must list credential IOC anchor for phishing_pull');
    }

    /**
     * The old weak "If natural, try to obtain" directive
     * MUST be gone. Its replacement is the stage-aware match expression
     * tested above.
     */
    public function test_objective_no_weak_if_natural_directive(): void
    {
        $prompts = $this->builder->buildGeneratorPrompts($this->baseContext(), 'generic_user');
