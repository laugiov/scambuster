...
/**
 * PromptBuilder must inject a "## PRIORITY OVERRIDE"
...
final class PromptBuilderForbiddenIocsTest extends TestCase
{
    ...
    /**
     * V3 regression — when `forbidden_iocs` is empty
     * (the dominant case, ~99.75 % of replies), the prompt must NOT contain
     * the "## PRIORITY OVERRIDE" section.
     */
    public function testPriorityOverrideAbsentWhenForbiddenIocsEmpty(): void
    {
    ...
    /**
     * V3 — when `forbidden_iocs` is populated, the
     ...
     */
    public function testPriorityOverridePresentWhenForbiddenIocsPopulated(): void
    {
        $builder = $this->buildBuilderWithAnalysisInstructions([
    ...
    /**
     * V3 — when the analyzer provides a `pivot_to_iocs`
     * list, the override block must use it as the suggested alternatives.
     * When absent, the default alternative list is used.
     */
    public function testPriorityOverridePivotListIncludesAnalyzerSuggestions(): void
    {
        $builder = $this->buildBuilderWithAnalysisInstructions([
            'interdictions' => [],
            'obligations' => [],
    ...
    /**
     * V3 — the OBJECTIVE section must be followed by
     * PRIORITY OVERRIDE (PRIORITY OVERRIDE is the LAST instructional block,
     * benefiting from recency bias).
     */
    public function testPriorityOverrideAppearsAfterObjective(): void
    {
        $builder = $this->buildBuilderWithAnalysisInstructions([
            'interdictions' => [],
            'obligations' => [],
    ...
    /**
     * V4 regression — when `forbidden_iocs` is empty,
     ...
     * happy path.
     */
    public function testBaselineObjectiveStillPullsIocsWhenForbiddenIocsEmpty(): void
    {
        $builder = $this->buildBuilderWithAnalysisInstructions([
            'interdictions' => [],
            'obligations' => [],
        ...
        // Existing payment_push OBJECTIVE wording must remain intact
        $this->assertStringContainsString('Stage: payment push', $userPrompt);
        $this->assertStringContainsString('IBAN', $userPrompt, 'Baseline OBJECTIVE must still mention IBAN (behavior preserved)');
