...
final class StixSchemaValidationTest extends TestCase
{
    ...
    // ---------------------------------------------------------------
    // TTP attack-patterns from TtpAttackPatternBuilder
    // ---------------------------------------------------------------

    public function testTtpAttackPatternsRequireKillChainPhases(): void
    {
        $builder = new TtpAttackPatternBuilder();
        $patterns = $builder->buildAttackPatterns([
            [
                'code' => 'SB-T013',
                'label' => 'Payment instrument designation',
                'definition' => 'Provides concrete payment coordinates.',
                'phase' => 'payment-request',
                'external_refs' => [['source_name' => 'mitre-attack', 'external_id' => 'T1566']],
            ],
        ]);
        ...
        foreach ($patterns as $ap) {
            $this->assertSame('attack-pattern', $ap['type']);
            $this->assertSame('2.1', $ap['spec_version']);
            $this->assertArrayHasKey('kill_chain_phases', $ap, 'TTP attack-pattern must carry kill_chain_phases');
            $this->assertIsArray($ap['kill_chain_phases']);
            /** @var list<array<string, mixed>> $phases */
            $phases = $ap['kill_chain_phases'];
            $this->assertNotEmpty($phases);
            $this->assertSame('scambuster-scam-phases', $this->str($phases[0]['kill_chain_name'] ?? null));
            $this->assertSame('payment-request', $this->str($phases[0]['phase_name'] ?? null));
        }
    }

    // ---------------------------------------------------------------
    // x_scambuster_ttp_sighting extension conforms to its schema
    // ---------------------------------------------------------------

    public function testTtpSightingExtensionConformsToSchema(): void
    {
        $builder = new TtpAttackPatternBuilder();
        $objects = $builder->buildClusterTtpObjects(
            [[
                'code' => 'SB-T013',
                'label' => 'Payment instrument designation',
                'definition' => 'Provides concrete payment coordinates.',
            ...
            'threat-actor--11111111-1111-4111-8111-111111111111',
            'dddddddd-0000-4000-8000-0000000000c1',
            '2026-07-30T12:00:00.000Z',
        );

        $validator = new ExtensionSchemaValidator();

        // A bundle carrying the sighting validates clean.
        $this->assertSame([], $validator->validateBundle(['objects' => $objects]));

        // Extract the extension payload and check it directly against the schema.
        $ext = null;
        ...
        $this->assertSame([], $validator->validate($ext, 'x_scambuster_ttp_sighting'));

        // A malformed payload (missing required cluster_id) is rejected.
        $this->assertNotSame([], $validator->validate(['schema_version' => '1.0'], 'x_scambuster_ttp_sighting'));
    }
