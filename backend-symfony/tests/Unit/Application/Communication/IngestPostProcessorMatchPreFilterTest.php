...
/**
 * Unit tests for IngestPostProcessor::matchPreFilter.
 *
 * Pinned cases (13):
...
final class IngestPostProcessorMatchPreFilterTest extends TestCase
{
    ...
    public function test_matches_dmarc_subject_regardless_of_sender(): void
    {
        $match = $this->createProcessor()->matchPreFilter(
            ...
            'Report Domain: acme-sourcing.example Submitter: protection.outlook.com Report-ID: abc',
        );
    ...
    public function test_matches_operator_test_sender(): void
    {
        $proc = $this->createProcessor(
            operatorTestSenders: 'operator-mgmt@example.com,operator-test@example.com',
        );
        $match = $proc->matchPreFilter('operator-mgmt@example.com', 'test first mail');
        self::assertNotNull($match);
        self::assertSame(PreFilterMatch::KIND_OPERATOR_TEST, $match->kind);
        self::assertSame('operator-mgmt@example.com', $match->pattern);
    }
