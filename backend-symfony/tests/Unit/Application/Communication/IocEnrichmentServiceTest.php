...
/**
 * Unit tests for IocEnrichmentService::calculateMessageRisk.
 *
...
final class IocEnrichmentServiceTest extends TestCase
{
    ...
    private function mockMessageWithScamType(string $scamCode, ConversationStatus $status = ConversationStatus::OPEN, int $convScoreRisk = 0): Message
    {
        ...
        // calculateMessageRisk now reads conv.status to short-circuit
        ...
        // conv-level score_risk is the engagement-continuity signal.
    ...
    public function test_calculateMessageRisk_returns_no_iocs_when_message_has_no_iocs(): void
    {
        ...
        // This is the path that legitimate-sender filtering
        // will rely on.
        $message = $this->mockMessageWithScamType('UNKNOWN');
    ...
    // ─── pre-filter shortcut on /risk endpoint ─────────
    ...
    private function mockPreFilteredMessageWithBodyIocs(string $kind = 'domain', string $pattern = 'github.com'): Message
    {
        ...
        $conversation = $this->createMock(Conversation::class);
        $conversation->method('getScamType')->willReturn($scamType);
        // production code reads conv.status before pre-filter check.
    ...
    {
        // pre-filter marker on headers must override any
    ...
    {
        // regression guard: messages without the marker
        // (commercial B2B, real scams, all the unfiltered cases) must
        // continue through the existing scoring path.
    ...
    {
        // defensive: a malformed marker (empty array,
    ...
    // ─── closed-conv short-circuit on /risk endpoint ──────────

    public function test_calculateMessageRisk_shortcuts_when_conversation_is_closed(): void
    {
        // when the operator has manually closed the conv,
        ...
        // before findBy, mirroring the pre-filter pattern.
        $messageRepo = $this->createMock(EntityRepository::class);
        $messageRepo->method('find')->willReturn($message);
    ...
    {
        // same short-circuit applies to abandoned and
    ...
    public function test_continuity_override_fires_on_engaged_conv(): void
    {
    ...
    public function test_continuity_override_does_not_fire_on_cold_start(): void
    {
        ...
        // preserved — exactly the behaviour the cold-start filter protects against).
    ...
    public function test_continuity_override_does_not_touch_high_paths(): void
    {
    ...
    public function test_continuity_override_respects_closed_conv_short_circuit(): void
    {
