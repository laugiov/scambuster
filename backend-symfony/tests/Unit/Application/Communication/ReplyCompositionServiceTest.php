...
class ReplyCompositionServiceTest extends TestCase
{
    ...
    // --- buildMessageId (Message-ID de-branding) ---

    public function test_buildMessageId_derives_domain_from_from_address(): void
    {
        $id = $this->createService()->buildMessageId('"Jane Doe" <jane@bank.example>');

        $this->assertMatchesRegularExpression('/^<[0-9a-f]{32}@bank\.example>$/', $id);
        $this->assertStringNotContainsStringIgnoringCase('scambuster', $id);
    }

    public function test_buildMessageId_handles_bare_from_address(): void
    {
        $id = $this->createService()->buildMessageId('agent@acme.co.uk');

        $this->assertStringEndsWith('@acme.co.uk>', $id);
    }

    public function test_buildMessageId_handles_display_name_containing_at_sign(): void
    {
        // RFC-compliant parsing must ignore an "@" inside the display name and
        // still resolve the real address domain (a naive regex returned garbage).
        $id = $this->createService()->buildMessageId('"a@b" <jane@bank.example>');

        $this->assertStringEndsWith('@bank.example>', $id);
        $this->assertStringNotContainsString('b"', $id);
    }

    public function test_buildMessageId_honors_explicit_domain_override(): void
    {
        $id = $this->createService(messageIdDomain: 'mail.operator.test')
            ->buildMessageId('jane@bank.example');

        $this->assertStringEndsWith('@mail.operator.test>', $id);
    }

    public function test_buildMessageId_falls_back_to_neutral_domain_when_unparseable(): void
    {
        $id = $this->createService()->buildMessageId('');

        $this->assertStringEndsWith('@localhost.localdomain>', $id);
        $this->assertStringNotContainsStringIgnoringCase('scambuster', $id);
    }
    ...
    // markAsSent idempotency on match, typed conflict on mismatch.
    ...
    public function test_markAsSent_throws_typed_conflict_when_stored_id_is_null(): void
    {
        ...
        // (older messages). Any non-empty input is by definition a
