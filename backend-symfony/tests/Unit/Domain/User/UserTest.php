...
final class UserTest extends TestCase
{
    ...
    /**
     * Storing a campaign rule is a WRITE action (it creates a database-executed
     * hunt query) and must NOT be reachable with a read-only grant. A principal
     * holding campaign:read but not campaign:write is denied.
     */
    public function testCampaignReadDoesNotImplyCampaignWrite(): void
    {
        $user = new User();
        $user->setRoles(['ROLE_USER']);
        $user->setPermissions(['campaign:read']);

        $this->assertTrue($user->hasPermission(Permission::CAMPAIGN_READ));
        $this->assertFalse(
            $user->hasPermission(Permission::CAMPAIGN_WRITE),
            'campaign:read must not confer campaign:write'
        );
    }
