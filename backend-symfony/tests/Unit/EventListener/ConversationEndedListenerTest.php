...
class ConversationEndedListenerTest extends TestCase
{
    ...
    public function testUsesRewardOverrideForLearningWhenPresent(): void
    {
        // The hybrid reward carried on the event must be used verbatim, not the
        // mechanical recomputation from the raw metrics.
        $event = new ConversationEndedEvent(
            'conv-ovr', 'ROMANCE', 'elderly_person', 7200, 20, 10, 5, true, 0.123,
        );
        ...
        $stats = $this->createMock(PersonaPerformanceStatsEntity::class);
        $stats->method('getSessionsCount')->willReturn(5);
        $stats->method('getRewardAvg')->willReturn(0.5);
        $stats->expects($this->once())->method('addReward')->with(0.123);
        $this->statsRepo->method('findOrCreate')->willReturn($stats);
