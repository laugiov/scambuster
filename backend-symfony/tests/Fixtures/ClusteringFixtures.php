...
final class ClusteringFixtures
{
    ...
    /**
     * Seed the fuzzy-matching scenario (G2). Three pairs of conversations, each
     * pair carrying a DIFFERENT indicator row (distinct value_norm) so exact match
     * would miss them:
     *  - f1/f2: same ETH wallet, checksum-case vs lowercase → MUST merge (canonical).
     *  - f3/f4: same IBAN, no-separators vs dashed → MUST merge (canonical).
     *  - f5/f6: two IBANs differing by one digit → MUST NOT merge.
     * Uses the cccccccc-/dddddddd-/eeeeeeee-/ffffffff- prefixes so cleanup() covers it.
     *
     * @return array<string, string> key (f1..f6) → conv_id
     */
    public static function loadFuzzyScenario(Connection $conn): array
    {
        $now = (new \DateTimeImmutable())->format('Y-m-d H:i:s');
        $scamTypeId = $conn->fetchOne('SELECT scam_type_id FROM lkp_scam_type LIMIT 1');
        $channelId = $conn->fetchOne('SELECT channel_id FROM lkp_channel LIMIT 1');
        ...
        /** @var array<int, array{type: string, value: string, valueNorm: string}> $anchors 1-indexed by f-number */
        $anchors = [
            1 => ['type' => 'wallet_eth', 'value' => '0xAbCdEf0123456789012345678901234567890AbC', 'valueNorm' => '0xAbCdEf0123456789012345678901234567890AbC'],
            2 => ['type' => 'wallet_eth', 'value' => '0xabcdef0123456789012345678901234567890abc', 'valueNorm' => '0xabcdef0123456789012345678901234567890abc'],
            3 => ['type' => 'iban', 'value' => 'FR7630001007941234567890185', 'valueNorm' => 'FR7630001007941234567890185'],
            4 => ['type' => 'iban', 'value' => 'FR76-3000-1007-9412-3456-7890-185', 'valueNorm' => 'FR76-3000-1007-9412-3456-7890-185'],
            5 => ['type' => 'iban', 'value' => 'DE89370400440532013000', 'valueNorm' => 'DE89370400440532013000'],
            6 => ['type' => 'iban', 'value' => 'DE89370400440532013001', 'valueNorm' => 'DE89370400440532013001'],
        ];

        $convs = [];

        foreach ($anchors as $n => $anchor) {
            $convId = self::createConversation($conn, sprintf('cccccccc-ffff-4000-8000-%012d', $n), $scamTypeId, $channelId, $accountId, $personaId, $now, 30 + $n);
            $msgId = self::createMessage($conn, sprintf('dddddddd-ffff-4000-8000-%012d', $n), $convId, $now);
            $indicatorId = self::createIndicator($conn, sprintf('eeeeeeee-ffff-4000-8000-%012d', $n), $anchor['type'], $anchor['value'], $anchor['valueNorm'], $now);
            self::createObservedIoc($conn, sprintf('ffffffff-ffff-4000-8000-%012d', $n), $msgId, $indicatorId, $anchor['type'], $anchor['value'], $now);
            $convs["f{$n}"] = $convId;
        }
