...
/**
 * Enhanced integration tests for ProfileCampaignHandler
 *
 * Covers:
 * - Message validation (0, 1, 2, 3+ messages)
 * - Sample size (3, 5, 10, 20 messages)
 * - profile_yaml storage in DB
 * - Cache behavior (same campaign called twice)
 * - Realistic scenarios (PayPal, banks with real headers)
 * - Edge cases (empty messages, unicode, very long)
 * - Persistence (read after storage)
 */
class ProfileCampaignHandlerEnhancedTest extends KernelTestCase
{
    ...
    {
        // 5 messages, requests 10 → should use the 5
        $campaignId = $this->createCampaignWithMessages(5);
    ...
    public function testHandleStoresProfileYamlInCampaign(): void
    {
        ...
        // Verify that profile_yaml was stored
        $this->em->clear();
    ...
    public function testHandleUpdatesExistingProfileYaml(): void
    {
        ...
        // Second profiling (should use cache or re-profile)
        $result2 = $this->handler->handle(Uuid::fromString($campaignId));

        // Both results should have profile_yaml
    ...
    public function testHandlePayPalPhishingCampaign(): void
    {
        ...
        // Verify that the profile is stored
        $this->em->clear();
        $campaign = $this->em->find(Campaign::class, Uuid::fromString($campaignId));
        $this->assertNotNull($campaign->getProfileYaml());
    }
    ...
    public function testHandleCompletesWithin30Seconds(): void
    {
        ...
        // Should complete in under 30s (FakeLLM is fast)
