...
/**
 * Integration tests for the complete Campaign Radar workflow (Phases 2-5).
 *
 * Scenarios tested:
 * - Hunter executes shadow rules and collects metrics
 * - Rules reach promotion thresholds
 * - Manual promotion via CampaignPromoter
 * - STIX export with all IoC types
 * - Campaign state handling (shadow → promoted → archived)
 */
class CampaignWorkflowIntegrationTest extends KernelTestCase
{
    ...
    {
        // ===== Phase 2-3: Create shadow campaign with rule =====
        ...
        // ===== Phase 4: Hunter executes rule and updates metrics =====
        $result = $this->hunter->hunt();
        
        $this->assertIsArray($result);
        $this->assertIsArray($result);
        
        // Simulate metrics collection after several runs
        ...
        // ===== Phase 5a: Evaluate promotion candidates =====
    ...
    public function testPromotionThresholdsValidation(): void
    {
        ...
        $ruleLowPPV->enable();
        $ruleLowPPV->updateMetrics(10, 7, 3); // PPV=0.7 < 0.85 (below threshold)
        $this->em->persist($ruleLowPPV);

        // Rule with insufficient hits
        ...
        $ruleLowHits->enable();
        $ruleLowHits->updateMetrics(3, 3, 0); // PPV=1.0 but hits=3 < 5
        $this->em->persist($ruleLowHits);

        $this->em->flush();

        // Attempt to promote rule with PPV too low
    ...
    public function testPromotionRequiresMinimumHits(): void
    {
        ...
        $rule->enable();
        $rule->updateMetrics(3, 3, 0); // PPV=1.0 but hits=3 < MIN_HITS(5)
        $this->em->persist($rule);
        $this->em->flush();
    ...
    public function testSTIXExportWithMultipleIoCTypes(): void
    {
        ...
        // Verify presence of different IoC types
    ...
    public function testCampaignStateTransitions(): void
    {
        ...
        // Attempt promotion after archive (should fail)
    ...
    public function testHunterExecutesOnlyEnabledRules(): void
    {
        ...
        // Verify that only the enabled rule was executed
