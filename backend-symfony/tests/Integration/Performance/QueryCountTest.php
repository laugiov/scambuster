...
class QueryCountTest extends KernelTestCase
{
    ...
    public function testIngestHandlerQueryCountBounded(): void
    {
        ...
        // Upper bound: 130 queries for a single message with 3 embedded IOCs.
        // Raised from 120: the in-process TTP extraction hook adds a fixed block
        // of queries to the pipeline (one taxonomy load, plus one observation
        // upsert and one audit write per extracted TTP). The guard stays
        // meaningful — a genuine per-IOC N+1 would scale multiplicatively and
        // blow well past this bound.
        $this->assertLessThanOrEqual(130, $queryCount, sprintf(
