...
/**
 * Integration coverage on the in-flight pull tracking.
 *
...
final class PersonaOptimizerInflightTest extends KernelTestCase
{
    ...
    {
        // Acceptance scenario 3 — in-flight on PHISHING must
    ...
    {
        // Surface check that the optimizer's audit log
        ...
        //
        // Without the fix: A wins (UCB1 bonus inflates A's 5-session
        // score above B's 19-session score).
        // With the fix: B wins (A's effective N = 22 ≈ B's 19, and
        ...
        // After the fix, exploit branch must pick personaB; we assert
        ...
        // With the fix, persona B should be picked significantly
        ...
        $this->assertGreaterThan(
            ...
            $selections['B'],
            sprintf(
                'Persona B (higher reward) must beat persona A (stuck with inflated bonus): got A=%d, B=%d',
                $selections['A'],
                $selections['B'],
            ),
        );
    ...
    {
        // Acceptance scenario 2 — without any in-flight
    ...
    {
        // When ALL personas are in cold start (no closed
