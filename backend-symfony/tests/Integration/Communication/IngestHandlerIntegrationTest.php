...
class IngestHandlerIntegrationTest extends KernelTestCase
{
    ...
    // ── Config-driven onboarding: derive the account from the recipient ──

    public function testIngestDerivesAccountFromRecipientWhenAccountIdEmpty(): void
    {
        $accountId = 'dddddddd-0000-0000-0000-00000000e001';
        $recipient = 'derive-trap@scambuster.test';
        $this->ensureMailAccount($accountId, $recipient);

        $raw = $this->buildRfc822Email(
            to: $recipient,
            messageId: '<derive-' . bin2hex(random_bytes(8)) . '@evil.test>',
        );
        $dto = $this->buildDto($raw);
        $dto->account_id = ''; // no explicit id — must be derived from the recipient
        ...
        $conv = $this->em->getRepository(\App\Domain\Communication\Conversation::class)->find($result['conv_id']);
        $this->assertNotNull($conv);
        $this->assertSame($accountId, $conv->getAccount()->getAccountId());
    }

    public function testIngestThrowsWhenRecipientMatchesNoAccount(): void
    {
        $raw = $this->buildRfc822Email(to: 'unknown-mailbox@nowhere.test');
        $dto = $this->buildDto($raw);
        $dto->account_id = '';
    ...
    private function ensureMailAccount(string $accountId, string $email): void
    {
        if ($this->em->getRepository(MailAccount::class)->find($accountId) !== null) {
            return;
        }
