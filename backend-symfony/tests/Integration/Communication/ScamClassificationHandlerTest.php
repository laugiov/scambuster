...
final class ScamClassificationHandlerTest extends KernelTestCase
{
    ...
    /**
     * Updated: confidence threshold lowered from 0.75 to 0.55.
    ...
    public function testItThrowsExceptionWhenConfidenceTooLow(): void
    {
        ...
        $classificationResult = new ClassificationResult(
            scamTypeCode: 'phishing',
            confidence: 0.40, // Below new threshold of 0.55
            isNewType: false,
            isNewPersona: false,
            personaCode: null,
    ...
    /**
     * confidence=0.65 (which was rejected under the old
     ...
     *
     */
    public function testItAcceptsConfidence065_PreviouslyRejected(): void
    {
        $conversation = $this->em->getRepository(Conversation::class)->findOneBy(['deletedAt' => null]);
        $this->assertNotNull($conversation);
        $convId = $conversation->getConvId();
