...
class ConversationServiceTest extends KernelTestCase
{
    ...
    public function testCreateConversationWithInvalidChannel(): void
    {
        ...
        // Force an incorrect value for the channel (e.g. empty object) to trigger a TypeError
    ...
    public function testAddChannelToConversationTwice(): void
    {
        ...
        $this->assertCount(1, $links, 'There must be only one conversation/channel link');
    }
    ...
    public function testChangeStatusOnNonexistentConversation(): void
    {
        ...
        $this->assertNull($result, 'Must return null if the conversation does not exist');
    }
    ...
    public function testMessageCreationUpdatesConversationTsLast(): void
    {
        ...
        // Create a message with a later date
    ...
    public function testConversationReceivesMultipleMessagesAndUpdatesTsLast(): void
    {
        ...
        $this->assertCount(2, $messages, 'The conversation must have 2 messages');
        foreach ($messages as $msg) {
            $this->assertSame($conv->getConvId(), $msg->getConversation()->getConvId(), 'All messages must be linked to the same conversation');
        }
        $this->assertEquals(
            ...
            'ts_last must be updated with the date of the last message'
        );
    }
    ...
    /**
     * Soft-deleted conversations must not leak into the list endpoint.
