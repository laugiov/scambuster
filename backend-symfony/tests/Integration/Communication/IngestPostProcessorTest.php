...
class IngestPostProcessorTest extends KernelTestCase
{
    ...
    {
        // Explicit out-of-scope: B2B commercial pitches must
    ...
    // ─── pre-filter decision marker on headers ──────────────

    public function testProcessAfterIngest_writesPreFilterMarkerOnHeaders(): void
    {
        // The pre-filter must persist a structured
        ...
        $message->setSubject('Report Domain: gamma-partners.example Submitter: protection.outlook.com');
        $this->em->flush();
        $conversation = $message->getConversation();
    ...
    {
        // Regression guard: B2B commercial mails (NOT pre-filtered
        // as out-of-scope) must NOT get the marker. The /risk
    ...
    // ─────────────────────────────────────────────────────────────
    //  TTP extraction at ingest time (in-process, no external workflow)
    // ─────────────────────────────────────────────────────────────

    public function testProcessAfterIngestPersistsTtpObservations(): void
    {
        // The container's TtpHandler runs against the FakeLLMClient in the test
        // env, whose ttp_extraction branch returns SB-T017 (0.92) and SB-T022
        // (0.4): one confirmed, one review — proving the whole in-process chain
        // (ingest hook → handler → extractor → upsert) without any n8n step.
        $message = $this->createTestMessage(
            'You must act now, there is no time for contracts, wire the fee today.',
            [
                'from' => 'scammer@ttp-ingest.test',
                'to' => 'honeypot@test.com',
                'message-id' => '<ttp-ingest-' . bin2hex(random_bytes(8)) . '@test.com>',
            ],
        );
        $conversation = $message->getConversation();
        ...
        /** @var list<array{status: string, confidence: string, code: string}> $rows */
        $rows = $connection->fetchAllAssociative(
            'SELECT o.status, o.confidence, t.code
        ...
        $this->assertCount(2, $rows, 'Both fake-LLM TTP items must be persisted at ingest time');
        $this->assertSame('SB-T017', $rows[0]['code']);
        $this->assertSame('confirmed', $rows[0]['status']);
        $this->assertSame(0.92, (float) $rows[0]['confidence']);
        $this->assertSame('SB-T022', $rows[1]['code']);
        $this->assertSame('review', $rows[1]['status']);
        $this->assertSame(0.4, (float) $rows[1]['confidence']);
    }

    public function testProcessAfterIngestSurvivesAMissingTtpHandler(): void
    {
        // The dependency is optional: a processor wired without the TTP handler
        // (partial deployment) must process the message normally.
        $processor = new IngestPostProcessor(
            em: $this->em,
            logger: $this->processorLogger(),
            iocHandler: $this->iocHandler,
        );

        $message = $this->createTestMessage('Plain message with no TTP handler wired.');

        $processor->processAfterIngest($message, $message->getConversation(), 'en');
    ...
    private function processorLogger(): \Psr\Log\LoggerInterface
    {
