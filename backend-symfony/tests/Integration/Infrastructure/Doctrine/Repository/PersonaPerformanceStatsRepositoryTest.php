...
final class PersonaPerformanceStatsRepositoryTest extends KernelTestCase
{
    ...
    {
        // Arrange: create an entity in the database
    ...
    public function testDeleteAllForPersonaRemovesEveryScamTypeRowAndIsIdempotent(): void
    {
        $persona = $this->em->getRepository(Persona::class)->findOneBy(['personaCode' => 'elderly_person']);
        $phishing = $this->em->getRepository(ScamType::class)->findOneBy(['code' => 'PHISHING']);
        $romance = $this->em->getRepository(ScamType::class)->findOneBy(['code' => 'ROMANCE']);
        $other = $this->em->getRepository(Persona::class)->findOneBy(['personaCode' => 'generic_user']);

        // Stats for the target persona across two scam types + a row for another persona.
        $this->em->persist(new PersonaPerformanceStatsEntity($persona, $phishing, 5, 3.2, 0.64));
        $this->em->persist(new PersonaPerformanceStatsEntity($persona, $romance, 3, 1.5, 0.50));
        $this->em->persist(new PersonaPerformanceStatsEntity($other, $phishing, 2, 1.0, 0.50));
        $this->em->flush();
        $this->em->clear();

        $target = $this->em->getRepository(Persona::class)->findOneBy(['personaCode' => 'elderly_person']);

        // Act: reset the target persona's stats.
        $removed = $this->repository->deleteAllForPersona($target);

        // Assert: both target rows gone, the other persona untouched, idempotent on re-run.
        $this->assertSame(2, $removed);
        $this->em->clear();
        $this->assertCount(0, $this->repository->findAllByPersona($target));
        $otherPersona = $this->em->getRepository(Persona::class)->findOneBy(['personaCode' => 'generic_user']);
        $this->assertCount(1, $this->repository->findAllByPersona($otherPersona));
        $this->assertSame(0, $this->repository->deleteAllForPersona($target));
    }
