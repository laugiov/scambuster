...
/**
 * Backfill headers.message-id from headers.provider_msg_id
 * for historical SMTP outbounds.
 *
 * Background: the send path was migrated from Gmail-API to Symfony
 ...
 *
 * The forward case in sendEmail is fixed separately. This migration
 * backfills the historical rows so that:
 *  - the primary ThreadResolver lookup on headers.message-id succeeds
 *    immediately (faster than the runtime fallback)
...
final class Version2026051800100000 extends AbstractMigration
{
    ...
    {
        return 'Backfill headers.message-id from headers.provider_msg_id for SMTP outbounds';
    }
    ...
    public function down(Schema $schema): void
    {
        throw new IrreversibleMigration(
            'Backfilled headers.message-id values cannot be selectively '
